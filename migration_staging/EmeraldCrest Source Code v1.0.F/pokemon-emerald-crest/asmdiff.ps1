...
if (-Not (Test-Path ".\emeraldcrest.gba"))
{
    Write-Host "File 'emeraldcrest.gba' not found."
    Write-Host $help
    exit
}
...
&$objdump -D -bbinary -marmv4t -Mforce-thumb --start-address="$start_str" --stop-address="$end_str" .\emeraldcrest.gba > .\pokeemerald.dump
