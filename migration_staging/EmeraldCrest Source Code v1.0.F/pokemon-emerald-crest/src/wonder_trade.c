...
static u16 PickRandomSpecies() // picks only base forms
{
    ...
    species = sValidSpecies[Random() % NELEMS(sValidSpecies)];
    if (species == SPECIES_VICTINI) {
        if ((Random() <= UINT16_MAX * 0.04)) {
           return species;
        }
        else {
            goto REROLL;
    ...
    else if (species == SPECIES_JIRACHI) {
        if ((Random() <= UINT16_MAX * 0.04)) {
           return species;
        }
        else {
    ...
    else if (species == SPECIES_FLUTTER_MANE) {
        if ((Random() <= UINT16_MAX * 0.01)) {
           return species;
        }
        else {
            goto REROLL;
    ...
    else if (species == SPECIES_SANDY_SHOCKS) {
        if ((Random() <= UINT16_MAX * 0.01)) {
           return species;
        }
        else {
    ...
    else if (species == SPECIES_SLITHER_WING) {
        if ((Random() <= UINT16_MAX * 0.01)) {
           return species;
        }
        else {
...
bool32 IsMegaPreEvolution(u16 species, u16 heldStone, bool32 found)
{
    ...
    for (i = 0; i < EVOS_PER_MON; i++)
    {
        ...
        {
            if (gEvolutionTable[species][i].method == FORM_CHANGE_BATTLE_MEGA_EVOLUTION_ITEM && gEvolutionTable[species][i].param == heldStone)
                found = TRUE;
...
u16 GetValidWonderTradeItem(u16 item)
{
    ...
    if (item == ITEM_NONE
     ...
     || item == ITEM_GRISEOUS_ORB)
        ...
        goto ROLL;
    else if (item >= ITEM_TM62 && item <= ITEM_TM100)
        goto ROLL;
    else if (item >= ITEM_POKEVIAL && item <= ITEM_LUSTROUS_GLOBE)
        goto ROLL;
    else if (item >= ITEM_ABILITY_SHIELD && item <= ITEM_LOADED_DICE)
        goto ROLL;
    else if (item == ITEM_DYNAMAX_CANDY)
        goto ROLL;
