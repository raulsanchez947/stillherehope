...
static void PlayerNotOnBikeMoving(u8 direction, u16 heldKeys)
{
    ...
    if (!(gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_UNDERWATER) && (heldKeys & B_BUTTON || gSaveBlock2Ptr->autoRun == 1) && FlagGet(FLAG_SYS_B_DASH)
     && IsRunningDisallowed(gObjectEvents[gPlayerAvatar.objectEventId].currentMetatileBehavior) == 0)
    {
        if (heldKeys & B_BUTTON && gSaveBlock2Ptr->autoRun == 1)
        {
...
u8 CheckForObjectEventCollision(struct ObjectEvent *objectEvent, s16 x, s16 y, u8 direction, u8 metatileBehavior)
{
    ...
    if (gObjectEvents[objectEventId].graphicsId == OBJ_EVENT_GFX_WORMHOLE2 && gObjectEvents[objectEventId].invisible == TRUE) {
        return COLLISION_NONE;
    }
    
    if (ShouldJumpLedge(x, y, direction))
    {
        stats = GetGameStat(GAME_STAT_JUMPED_DOWN_LEDGES);
        if (stats >= 300 && stats % 20 == 0) {
            gSaveBlock2Ptr->playerRank += 5;
            #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "ledge jumped +5:%d", gSaveBlock2Ptr->playerRank);
            #endif
        }
