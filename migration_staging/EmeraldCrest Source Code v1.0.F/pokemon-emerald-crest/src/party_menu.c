...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    {
        if (hasFlashAlready == FALSE && CheckBagHasItem(ITEM_HM05_FLASH, 1) && CanTeachMove(&mons[slotId], MOVE_FLASH) != ALREADY_KNOWS_MOVE)
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 1 + MENU_FIELD_MOVES);
        if (hasFlyAlready == FALSE && CheckBagHasItem(ITEM_HM02_FLY, 1) && CanTeachMove(&mons[slotId], MOVE_FLY) != ALREADY_KNOWS_MOVE)
