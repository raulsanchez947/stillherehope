...
#include "tv.h"
#include "wild_encounter.h" // Chain Fishing 
...
void InitMap(void)
{
    gChainFishingStreak = 0; // Chain Fishing
...
void LoadTilesetPalette(struct Tileset const *tileset, u16 destOffset, u16 size, bool8 skipFaded)
{
    ...
    const u16 (*seasons[])[16] = {
        tileset->palettes,
        tileset->fall_palettes,
        tileset->winter_palettes,
        tileset->spring_palettes
    };
    
    GetCurrentSeason();

    if (seasons[gSaveBlock1Ptr->seasons] != NULL && FlagGet(FLAG_SEASONS))
    {
        palettes = seasons[gSaveBlock1Ptr->seasons];
        // MgbaPrintf(MGBA_LOG_DEBUG, "Season");
    }
    else 
    {
        palettes = tileset->palettes;
        // MgbaPrintf(MGBA_LOG_DEBUG, "Regular");   
    }
    // MgbaPrintf(MGBA_LOG_DEBUG, "Season: %d, daysUntilChange: %d", gSaveBlock1Ptr->seasons, GetDaysUntilSeasonChange());
    if (tileset)
    {
        ...
        {
            if (skipFaded)
                CpuFastCopy(&palettes[NUM_PALS_IN_PRIMARY], &gPlttBufferUnfaded[destOffset], size);
            else
