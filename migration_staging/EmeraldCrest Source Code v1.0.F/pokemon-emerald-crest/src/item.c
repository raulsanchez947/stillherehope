...
void DrawHeaderBox(void)
{
    ...
    u8 headerType = VarGet(VAR_UNUSED_0x40FE);
    u8 textY;
    u8 *dst;
...
u16 RandomItemId(u16 itemId)
{
    ...
    if (!FlagGet(FLAG_ADVENTURE_STARTED) && itemId == ITEM_POKE_BALL) {
        return itemId;
    }

    if ((itemId == ITEM_RARE_CANDY || itemId == ITEM_MASTER_BALL) && VarGet(VAR_ROUTE111_STATE) == 4)
    {
        return itemId;
    }
    ...
    if (ItemId_GetPocket(itemId) == POCKET_TM_HM)
    {
        ...
        {
            u8 i;
            itemId = ITEM_TM01 + RandomSeededModulo(itemId, 61);
            for (i = 0; i < 255; i++)
            {
