...
{
    if (!FlagGet(FLAG_CAUGHT_HOOPA))
    {
        ROAMER->species = SPECIES_HOOPA;
    } else {
    ...
    ROAMER->level = 40;
    ROAMER->status = 0;
    ROAMER->item = ITEM_NONE;
    if (ROAMER->species == SPECIES_HOOPA) {
        ROAMER->item = ITEM_PRISON_BOTTLE;
    }
    ROAMER->active = TRUE;
...
void InitRoamer(void)
{
    ...
    CreateInitialRoamerMon(VAR_ROAMER_POKEMON);
}
...
#endif
    SetMonData(mon, MON_DATA_HELD_ITEM, &ROAMER->item);
...
{
    if (ROAMER->species == SPECIES_HOOPA) {
        FlagSet(FLAG_CAUGHT_HOOPA);
    }
    if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_HOOPA), FLAG_GET_CAUGHT)) {
        FlagClear(FLAG_CAUGHT_HOOPA);
    }

    if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_LATIOS), FLAG_GET_CAUGHT) && FlagGet(FLAG_CAUGHT_LATIAS_OR_LATIOS)) {
