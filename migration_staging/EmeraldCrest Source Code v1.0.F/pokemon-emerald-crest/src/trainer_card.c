...
static const u8 sTrainerCardSeedColors[] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_LIGHT_GREEN, TEXT_COLOR_GREEN};
...
const u8 sText_Seed[] = _("SEED: ");

u8 count_digits(s32 num) {
    u8 count = 0;
    while (num != 0) {
        num = num / 10;
        count++;
...
static void PrintIdOnCard(void)
{
    ...
    s32 xPos;
    u32 top;
    s32 randomizerSeed = GetTrainerId(gSaveBlock2Ptr->playerTrainerId);

   
    txtPtr2 = StringCopy(buffer2, sText_Seed);    
    ConvertIntToDecimalStringN(txtPtr2, randomizerSeed, STR_CONV_MODE_LEADING_ZEROS, count_digits(randomizerSeed));
    
    // txtPtr = StringCopy(buffer, gText_TrainerCardIDNo);
    // ConvertIntToDecimalStringN(txtPtr, sData->trainerCard.trainerId, STR_CONV_MODE_LEADING_ZEROS, 5);
    if (sData->cardType == CARD_TYPE_FRLG)
    {
        xPos = GetStringCenterAlignXOffset(FONT_NORMAL, buffer2, 80) + 132;
        top = 8;
    }
    else
    {
        xPos = GetStringCenterAlignXOffset(FONT_NORMAL, buffer2, 96) + 120;
        top = 8;
    }

    AddTextPrinterParameterized3(WIN_CARD_TEXT, FONT_NORMAL, xPos, top, sTrainerCardSeedColors, TEXT_SKIP_DRAW, buffer2);
...
static const u8 sText_Rank[] = _("Rank: {STR_VAR_1}");
static void BufferNameForCardBack(void)
{
    u8 diff;
    ...
    if (sData->cardType != CARD_TYPE_FRLG)
    {
        ...
        if (diff == EXTREME) {
            StringCopy(gStringVar2, sText_Extreme);            
        } 
        else if (diff == HARD) {
            StringCopy(gStringVar2, sText_Hard);            
        }
        else if (diff == NORMAL) {
            StringCopy(gStringVar2, sText_Normal);            
        }
        else if (diff == EASY) {
            StringCopy(gStringVar2, sText_Easy);            
        } 
...
static void PrintNameOnCardBack(void)
{
    u8 rank = CheckPlayerRank();

        if (rank == LEGEND) {
            StringCopy(gStringVar1, gText_Legend);
        } 
        else if (rank == MASTER) {
            StringCopy(gStringVar1, gText_Master);
        }
        else if (rank == CHAMPION) {
            StringCopy(gStringVar1, gText_Champion);
        }
        else if (rank == ELITE) {
            StringCopy(gStringVar1, gText_Elite);
        }
        else if (rank == ADVENTURER) {
            StringCopy(gStringVar1, gText_Adventurer);
        }
        else {
            StringCopy(gStringVar1, gText_Rookie);
        }
        StringExpandPlaceholders(sData->TextPlayerRank, sText_Rank);
    if (!sData->isHoenn)
        ...
        AddTextPrinterParameterized3(WIN_CARD_TEXT, FONT_NORMAL, 2, 9, sTrainerCardTextColors, TEXT_SKIP_DRAW, sData->TextPlayerRank);
}
