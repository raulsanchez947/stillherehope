...
EWRAM_DATA struct DexNavSearch *gDexNavSearchDataPtr = NULL;   // Global declaration in dexnav.h
...
EWRAM_DATA static bool8 RunData = FALSE;
EWRAM_DATA static bool8 CallData = FALSE;
...
{
    return (GetWindowAttribute(gDexNavSearchDataPtr->windowId, WINDOW_TILEMAP_TOP) * 8);
}
...
static void DrawDexNavSearchMonIcon(u16 species, u8 *dst, bool8 owned)
{
    ...
    *dst = spriteId;
    ...
    if (owned)
        gDexNavSearchDataPtr->ownedIconSpriteId = CreateSprite(&sOwnedIconTemplate, SPECIES_ICON_X + 6, GetSearchWindowY() + 4, 0);
}
...
static void AddSearchWindow(u8 width)
{
    ...
    if (gDexNavSearchDataPtr->tileY > (gSaveBlock1Ptr->pos.y + 7))
        y = 1;  //draw at top if chosen tile is below

    LoadDexNavWindowGfx(gDexNavSearchDataPtr->windowId, 0x1d5, 14 * 16);
    
    SetWindowTemplateFields(&template, 0, 1, y, width, 3, 14, 8);
    
    gDexNavSearchDataPtr->windowId = AddWindow(&template);
    FillWindowPixelBuffer(gDexNavSearchDataPtr->windowId, PIXEL_FILL(1));
    PutWindowTilemap(gDexNavSearchDataPtr->windowId);
    CopyWindowToVram(gDexNavSearchDataPtr->windowId, 3);
    
    DrawStdFrameWithCustomTileAndPalette(gDexNavSearchDataPtr->windowId, TRUE, 0x214, 14);
}

#define WINDOW_COL_0        (SPECIES_ICON_X + 4)
#define WINDOW_COL_1        (WINDOW_COL_0 + (GetFontAttribute(gDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * (POKEMON_NAME_LENGTH)))
#define WINDOW_MOVE_NAME_X  (WINDOW_COL_1 + (GetFontAttribute(gDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * 6))
...
{
    u8 windowId = gDexNavSearchDataPtr->windowId;
    
    //species name - always present
    if (hidden)
    {
        StringCopy(gStringVar4, sText_ThreeQmarks);
        AddTextPrinterParameterized3(gDexNavSearchDataPtr->windowId, 0, WINDOW_COL_0, 0, sSearchFontColor, TEXT_SKIP_DRAW, gStringVar4);
        return;
    }
    else
    {
        StringCopy(gStringVar1, gSpeciesNames[species]);
        AddTextPrinterParameterized3(gDexNavSearchDataPtr->windowId, 0, WINDOW_COL_0, 0, sSearchFontColor, TEXT_SKIP_DRAW, gStringVar1);
    }
    
    //level - always present
    ConvertIntToDecimalStringN(gStringVar1, gDexNavSearchDataPtr->monLevel, STR_CONV_MODE_LEFT_ALIGN, 3);
    StringExpandPlaceholders(gStringVar4, sText_MonLevel);
    AddTextPrinterParameterized3(gDexNavSearchDataPtr->windowId, 0, WINDOW_COL_1, 0, sSearchFontColor, TEXT_SKIP_DRAW, gStringVar4);
    
    if (proximity <= SNEAKING_PROXIMITY)
    {
        PlaySE(SE_POKENAV_ON);
        // move
        if (searchLevel > 1 && gDexNavSearchDataPtr->moves[0])
        {
            StringCopy(gStringVar1, gMoveNames[gDexNavSearchDataPtr->moves[0]]);
        ...
        {            
            // ability name
            StringCopy(gStringVar1, gAbilityNames[GetAbilityBySpecies(species, gDexNavSearchDataPtr->abilityNum)]);
            AddTextPrinterParameterized3(windowId, 0, WINDOW_COL_1 + 16, 12, sSearchFontColor, TEXT_SKIP_DRAW, gStringVar1);
        
            // item name
            if (gDexNavSearchDataPtr->heldItem)
            {
                CopyItemName(gDexNavSearchDataPtr->heldItem, gStringVar1);
    ...
    CopyWindowToVram(gDexNavSearchDataPtr->windowId, 2);
}

static void DrawSearchWindow(u16 species, u8 potential, bool8 hidden)
{
    u8 searchLevel = gDexNavSearchDataPtr->searchLevel;
    
    AddSearchWindow(28);
    AddSearchWindowText(species, gDexNavSearchDataPtr->proximity, searchLevel, hidden);
}
...
static void RemoveDexNavWindowAndGfx(void)
{
    ...
    // try remove sprites
    if (gDexNavSearchDataPtr->iconSpriteId != MAX_SPRITES)
        DestroySprite(&gSprites[gDexNavSearchDataPtr->iconSpriteId]);
    if (gDexNavSearchDataPtr->itemSpriteId != MAX_SPRITES)
        DestroySprite(&gSprites[gDexNavSearchDataPtr->itemSpriteId]);
    if (gDexNavSearchDataPtr->eyeSpriteId != MAX_SPRITES)
        DestroySprite(&gSprites[gDexNavSearchDataPtr->eyeSpriteId]);
    if (gDexNavSearchDataPtr->ownedIconSpriteId != MAX_SPRITES)
        DestroySprite(&gSprites[gDexNavSearchDataPtr->ownedIconSpriteId]);
    if (gDexNavSearchDataPtr->exclamationSpriteId != MAX_SPRITES)
        DestroySprite(&gSprites[gDexNavSearchDataPtr->exclamationSpriteId]);
    
    for (i = 0; i < NELEMS(gDexNavSearchDataPtr->starSpriteIds); i++)
    {
        if (gDexNavSearchDataPtr->starSpriteIds[i] != MAX_SPRITES)
            DestroySprite(&gSprites[gDexNavSearchDataPtr->starSpriteIds[i]]);
    }
    ...
    SafeFreeMonIconPalette(gDexNavSearchDataPtr->species);
    
    // remove window
    ClearStdWindowAndFrameToTransparent(gDexNavSearchDataPtr->windowId, FALSE);
    CopyWindowToVram(gDexNavSearchDataPtr->windowId, 3);
    RemoveWindow(gDexNavSearchDataPtr->windowId);
}
...
static void DexNavProximityUpdate(void)
{
    gDexNavSearchDataPtr->proximity = GetPlayerDistance(gDexNavSearchDataPtr->tileX, gDexNavSearchDataPtr->tileY);
}
...
static bool8 DexNavPickTile(u8 environment, u8 areaX, u8 areaY, bool8 smallScan)
{
    ...
    while (topY < botY)
    {
        while (topX < botX)
        {
            ...
            if (weight > 0)
            {
                gDexNavSearchDataPtr->tileX = topX;
                gDexNavSearchDataPtr->tileY = topY;
                return TRUE;
            }
...
static bool8 TryStartHiddenMonFieldEffect(u8 environment, u8 xSize, u8 ySize, bool8 smallScan)
{
    ...
    if (DexNavPickTile(environment, xSize, ySize, smallScan))
    {
        ...
        if (fldEffId != 0)
        {
            gFieldEffectArguments[0] = gDexNavSearchDataPtr->tileX;
            gFieldEffectArguments[1] = gDexNavSearchDataPtr->tileY;
            gFieldEffectArguments[2] = 0xFF; // subpriority
            gFieldEffectArguments[3] = 2;   //priority
                if (FlagGet(FLAG_SEASONS) && fldEffId == FLDEFF_SHAKING_GRASS) {
                    switch(gSaveBlock1Ptr->seasons)
                    {
                        case SEASON_SUMMER:
                            gDexNavSearchDataPtr->fldEffSpriteId = FieldEffectStart(FLDEFF_SHAKING_GRASS);
                            break;
                        break;
                        case SEASON_FALL:
                            gDexNavSearchDataPtr->fldEffSpriteId = FieldEffectStart(FLDEFF_SHAKING_GRASS_FALL);
                            break;
                        break;
                        case SEASON_WINTER:
                            gDexNavSearchDataPtr->fldEffSpriteId = FieldEffectStart(FLDEFF_SHAKING_GRASS_WINTER);
                            break;
                        break;
                        case SEASON_SPRING:
                            gDexNavSearchDataPtr->fldEffSpriteId = FieldEffectStart(FLDEFF_SHAKING_GRASS_SPRING);
                            break;
                        break;
                    }
                } else if (FlagGet(FLAG_SEASONS) && fldEffId == FLDEFF_SHAKING_LONG_GRASS) {
                    switch(gSaveBlock1Ptr->seasons)
                    {
                        case SEASON_SUMMER:
                            gDexNavSearchDataPtr->fldEffSpriteId = FieldEffectStart(FLDEFF_SHAKING_LONG_GRASS);
                            break;
                        break;
                        case SEASON_FALL:
                            gDexNavSearchDataPtr->fldEffSpriteId = FieldEffectStart(FLDEFF_SHAKING_GRASS2_FALL);
                            break;
                        break;
                        case SEASON_WINTER:
                            gDexNavSearchDataPtr->fldEffSpriteId = FieldEffectStart(FLDEFF_SHAKING_GRASS2_WINTER);
                            break;
                        break;
                        case SEASON_SPRING:
                            gDexNavSearchDataPtr->fldEffSpriteId = FieldEffectStart(FLDEFF_SHAKING_GRASS2_SPRING);
                            break;
                        break;
                    }
                } else {
                    gDexNavSearchDataPtr->fldEffSpriteId = FieldEffectStart(fldEffId);
                }
            if (gDexNavSearchDataPtr->fldEffSpriteId == MAX_SPRITES)
                return FALSE;
            
            gDexNavSearchDataPtr->fldEffId = fldEffId;
            return TRUE;
        }
    }
...
static void Task_SetUpDexNavSearch(u8 taskId)
{
    ...
    u16 species = gDexNavSearchDataPtr->species;
    u8 environment = gDexNavSearchDataPtr->environment;
    u8 searchLevel = gSaveBlock1Ptr->dexNavSearchLevels[SpeciesToNationalPokedexNum(species)];
    
    // init sprites
    gDexNavSearchDataPtr->iconSpriteId = MAX_SPRITES;
    gDexNavSearchDataPtr->itemSpriteId = MAX_SPRITES;
    gDexNavSearchDataPtr->eyeSpriteId = MAX_SPRITES;
    gDexNavSearchDataPtr->starSpriteIds[0] = MAX_SPRITES;
    gDexNavSearchDataPtr->starSpriteIds[1] = MAX_SPRITES;
    gDexNavSearchDataPtr->starSpriteIds[2] = MAX_SPRITES;
    gDexNavSearchDataPtr->ownedIconSpriteId = MAX_SPRITES;
    gDexNavSearchDataPtr->exclamationSpriteId = MAX_SPRITES;    
    gDexNavSearchDataPtr->searchLevel = searchLevel;
    
    DexNavGenerateMoveset(species, searchLevel, gDexNavSearchDataPtr->monLevel, &gDexNavSearchDataPtr->moves[0]);
    gDexNavSearchDataPtr->heldItem = DexNavGenerateHeldItem(species, searchLevel);
    gDexNavSearchDataPtr->abilityNum = DexNavGetAbilityNum(species, searchLevel);
    gDexNavSearchDataPtr->potential = DexNavGeneratePotential(searchLevel);
    DexNavProximityUpdate();
    
    LoadSearchIconData();
    if (gDexNavSearchDataPtr->hiddenSearch)
    {
    ...
    {
        DexNavDrawIcons();
        DexNavUpdateSearchWindow(gDexNavSearchDataPtr->proximity, searchLevel);
    }
    
    FlagSet(FLAG_SYS_DEXNAV_SEARCH);
    FlagSet(OW_FLAG_NO_ENCOUNTER);
    gDexNavSearchDataPtr->taskId = taskId;                                // Store taskId for use in start_menu.c

    if (gSaveBlock2Ptr->autoRun == TRUE) {
        gSaveBlock2Ptr->autoRun = FALSE;
        RunData = TRUE;
    }
    if (gSaveBlock2Ptr->optionsDisableMatchCall == FALSE)
    {
    ...
    gSaveBlock2Ptr->playerRank += 1;
    #ifndef NDEBUG
        MgbaPrintf(MGBA_LOG_DEBUG, "dexnav used +1:%d", gSaveBlock2Ptr->playerRank);
    #endif
}
...
static void Task_InitDexNavSearch(u8 taskId)
{
    ...
    gDexNavSearchDataPtr = AllocZeroed(sizeof(struct DexNavSearch));
    
    // assign non-objects to struct
    gDexNavSearchDataPtr->species = species;
    gDexNavSearchDataPtr->environment = environment;  //updated in DexNavTryGenerateMonLevel if hidden mon
    gDexNavSearchDataPtr->isHiddenMon = (environment == ENCOUNTER_TYPE_HIDDEN) ? TRUE : FALSE;
    gDexNavSearchDataPtr->monLevel = DexNavTryGenerateMonLevel(species, environment);
    ...
    if (gDexNavSearchDataPtr->monLevel == MON_LEVEL_NONEXISTENT || !TryStartHiddenMonFieldEffect(gDexNavSearchDataPtr->environment, 12, 12, FALSE))
    {
        if (RunData == TRUE) {
    ...
    gDexNavSearchDataPtr->hiddenSearch = FALSE;
...
{
    u16 tileX = gDexNavSearchDataPtr->tileX;
    u16 tileY = gDexNavSearchDataPtr->tileY;
...
static void DexNavDrawIcons(void)
{
    ...
    // init sprite ids
    /*gDexNavSearchDataPtr->iconSpriteId = 0xFF;
    gDexNavSearchDataPtr->itemSpriteId = 0xFF;
    for (i = 0; i < NELEMS(gDexNavSearchDataPtr->starSpriteIds); i++)
        gDexNavSearchDataPtr->starSpriteIds[i] = 0xFF;*/
    
    DrawSearchWindow(species, gDexNavSearchDataPtr->potential, FALSE);
    DrawDexNavSearchMonIcon(species, &gDexNavSearchDataPtr->iconSpriteId, GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT));
    DrawDexNavSearchHeldItem(&gDexNavSearchDataPtr->itemSpriteId);
    if (FlagGet(FLAG_PERFECT_IVS)) {
        DexNavDrawPotentialStars(3, &gDexNavSearchDataPtr->starSpriteIds[0]);
    } else {
        DexNavDrawPotentialStars(gDexNavSearchDataPtr->potential, &gDexNavSearchDataPtr->starSpriteIds[0]);
    }
    DexNavUpdateDirectionArrow();
    //DexNavDrawSight(gDexNavSearchDataPtr->proximity, &gDexNavSearchDataPtr->eyeSpriteId);
}
...
{
    if (gDexNavSearchDataPtr->searchLevel < 20)
        return 2;
    else if (gDexNavSearchDataPtr->searchLevel < 50)
        return 3;
    else
        return 4;
}
...
static void Task_RevealHiddenMon(u8 taskId)
{
    ...
    if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_SEEN))
    {
        ...
        DrawSearchWindow(species, gDexNavSearchDataPtr->potential, TRUE);
        DrawDexNavSearchMonIcon(species, &gDexNavSearchDataPtr->iconSpriteId, FALSE);
        // whiteout icon
        index = IndexOfSpritePaletteTag(gSprites[gDexNavSearchDataPtr->iconSpriteId].template->paletteTag);
        CpuCopy16(&gPlttBufferUnfaded[0x100 + index * 16], gDexNavSearchDataPtr->palBuffer, 32);
        TintPalette_CustomTone(gDexNavSearchDataPtr->palBuffer, 16, 510, 510, 510);
        LoadPalette(gDexNavSearchDataPtr->palBuffer, 0x100 + index * 16, 32);
    }
    else
    {
        DrawSearchWindow(species, gDexNavSearchDataPtr->potential, FALSE);
...
static void Task_DexNavSearch(u8 taskId)
{
    ...
    if (gDexNavSearchDataPtr->proximity > MAX_PROXIMITY)
    { // out of range
        if (gDexNavSearchDataPtr->hiddenSearch && !task->tRevealed)
            EndDexNavSearch(taskId);
        else
    ...
    if (gDexNavSearchDataPtr->proximity <= CREEPING_PROXIMITY && !gPlayerAvatar.creeping && task->tFrameCount > 60)
    ...
    if (gDexNavSearchDataPtr->proximity <= SNEAKING_PROXIMITY && TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_DASH | PLAYER_AVATAR_FLAG_BIKE)) 
    ...
    if (gDexNavSearchDataPtr->proximity < 1)
    {
        CreateDexNavWildMon(gDexNavSearchDataPtr->species, gDexNavSearchDataPtr->potential, gDexNavSearchDataPtr->monLevel, 
          gDexNavSearchDataPtr->abilityNum, gDexNavSearchDataPtr->heldItem, gDexNavSearchDataPtr->moves);
    ...
    if (gDexNavSearchDataPtr->hiddenSearch && !task->tRevealed &&
        (JOY_NEW(R_BUTTON) || (gDexNavSearchDataPtr->proximity < CREEPING_PROXIMITY)))
    {
        PlaySE(SE_DEX_SEARCH);
        ClearStdWindowAndFrameToTransparent(gDexNavSearchDataPtr->windowId, FALSE);
        CopyWindowToVram(gDexNavSearchDataPtr->windowId, 3);
        ...
        //gDexNavSearchDataPtr->hiddenSearch = FALSE; //now its a regular dexnav search
        ...
        && task->tRevealed)
    {
        ...
        gDexNavSearchDataPtr->movementCount++;
    }

    DexNavProximityUpdate();
    if (task->tProximity != gDexNavSearchDataPtr->proximity)
    {
        //player has moved
        if (task->tRevealed)    //update search window info only if hidden mon has been revealed (always true for search mode)
            DexNavUpdateSearchWindow(gDexNavSearchDataPtr->proximity, gDexNavSearchDataPtr->searchLevel);
        
        task->tProximity = gDexNavSearchDataPtr->proximity;
    }
...
/*static void DexNavSightUpdate(u8 index)
{
    u8 spriteId = gDexNavSearchDataPtr->eyeSpriteId;
...
static void DexNavUpdateSearchWindow(u8 proximity, u8 searchLevel)
{
    ...
    if (gDexNavSearchDataPtr->hiddenSearch && !GetSetPokedexFlag(SpeciesToNationalPokedexNum(gDexNavSearchDataPtr->species), FLAG_GET_SEEN))
        hideName = TRUE;    //if a detector mode hidden search and player hasn't seen the mon, hide info
    
    FillWindowPixelBuffer(gDexNavSearchDataPtr->windowId, PIXEL_FILL(1));   //clear window
    AddSearchWindowText(gDexNavSearchDataPtr->species, proximity, searchLevel, hideName);
    
    DexNavUpdateDirectionArrow();
    
    //init hidden sprites
    if (gDexNavSearchDataPtr->itemSpriteId != MAX_SPRITES)
        gSprites[gDexNavSearchDataPtr->itemSpriteId].invisible = TRUE;
    if (gDexNavSearchDataPtr->starSpriteIds[0] != MAX_SPRITES)
        gSprites[gDexNavSearchDataPtr->starSpriteIds[0]].invisible = TRUE;
    if (gDexNavSearchDataPtr->starSpriteIds[1] != MAX_SPRITES)
        gSprites[gDexNavSearchDataPtr->starSpriteIds[1]].invisible = TRUE;
    if (gDexNavSearchDataPtr->starSpriteIds[2] != MAX_SPRITES)
        gSprites[gDexNavSearchDataPtr->starSpriteIds[2]].invisible = TRUE;
    
    if (proximity <= SNEAKING_PROXIMITY)
    {
        if (searchLevel > 2 && gDexNavSearchDataPtr->heldItem)
        {
            // toggle item view
            if (gDexNavSearchDataPtr->itemSpriteId != MAX_SPRITES)
                gSprites[gDexNavSearchDataPtr->itemSpriteId].invisible = FALSE;
        }
        ...
        if (FlagGet(FLAG_PERFECT_IVS))
        {
           if (gDexNavSearchDataPtr->starSpriteIds[0] != MAX_SPRITES)
            ...
            if (gDexNavSearchDataPtr->starSpriteIds[0] != MAX_SPRITES)
                gSprites[gDexNavSearchDataPtr->starSpriteIds[0]].invisible = FALSE;

            if (gDexNavSearchDataPtr->starSpriteIds[1] != MAX_SPRITES)
                gSprites[gDexNavSearchDataPtr->starSpriteIds[1]].invisible = FALSE;

            if (gDexNavSearchDataPtr->starSpriteIds[2] != MAX_SPRITES)
                gSprites[gDexNavSearchDataPtr->starSpriteIds[2]].invisible = FALSE;
        }
    }
}
...
static void CreateDexNavWildMon(u16 species, u8 potential, u8 level, u8 abilityNum, u16 item, u16* moves)
{
    ...
    CreateDexnavWildMon(species, level);  // shiny rate bonus handled in CreateBoxMon
...
static u8 DexNavTryGenerateMonLevel(u16 species, u8 environment)
{
    ...
    u8 levelBonus = gSaveBlock1Ptr->dexNavChain / 10;

    if (levelBase == MON_LEVEL_NONEXISTENT)
        return MON_LEVEL_NONEXISTENT;   //species not found in the area
    
    if (Random() % 100 < 4)
        levelBonus += 5; //4% chance of having a +10 level

    if (levelBase + levelBonus > 50)
        return 50;
    else
...
static void DexNavGenerateMoveset(u16 species, u8 searchLevel, u8 encounterLevel, u16* moveDst)
{
    ...
    CreateDexnavWildMon(species, encounterLevel);
...
static u8 GetEncounterLevelFromMapData(u16 species, u8 environment)
{
    ...
    {    
        ...
        case ENCOUNTER_TYPE_LAND:    // grass
            ...
            {
                if (GetSpeciesRandomSeeded(landMonsInfo->wildPokemon[i].species, TX_RANDOM_T_WILD_POKEMON, 0) == species)
                {
                    if (FlagGet(FLAG_OPEN_WORLD))
                    {
                        return DynamicEncounterLevel();
                    }
        ...
        case ENCOUNTER_TYPE_WATER:    //water
            ...
            {
                if (GetSpeciesRandomSeeded(waterMonsInfo->wildPokemon[i].species, TX_RANDOM_T_WILD_POKEMON, 0) == species)
                {
                    if (FlagGet(FLAG_OPEN_WORLD))
                    {
                        return DynamicEncounterLevel();
        ...
        case ENCOUNTER_TYPE_HIDDEN:
            ...
            {
                if (GetSpeciesRandomSeeded(hiddenMonsInfo->wildPokemon[i].species, TX_RANDOM_T_WILD_POKEMON, 0) == species)
                {
                    if (FlagGet(FLAG_OPEN_WORLD))
                    {
                        min = DynamicEncounterLevel();
                        max = DynamicEncounterLevel();
                    } else {
            ...
            if (hiddenMonsInfo->encounterRate == 1)
                gDexNavSearchDataPtr->environment = ENCOUNTER_TYPE_WATER;
            else
                gDexNavSearchDataPtr->environment = ENCOUNTER_TYPE_LAND;
            break;
        default:
            return MON_LEVEL_NONEXISTENT;
        }
    } else if (FlagGet(FLAG_OPEN_WORLD)) {
        switch (environment)
        {
        case ENCOUNTER_TYPE_LAND:    // grass
            if (landMonsInfo == NULL)
...
static void DexNavLoadEncounterData(void)
{
    ...
    if (gSaveBlock1Ptr->tx_Random_WildPokemon)  
    {
        ...
        if (landMonsInfo != NULL && landMonsInfo->encounterRate != 0)
        {
            for (i = 0; i < LAND_WILD_COUNT; i++)
            {
                species = GetSpeciesRandomSeeded(landMonsInfo->wildPokemon[i].species, TX_RANDOM_T_WILD_POKEMON, 0);
        ...
        if (waterMonsInfo != NULL && waterMonsInfo->encounterRate != 0)
        {
            for (i = 0; i < WATER_WILD_COUNT; i++)
            {
                species = GetSpeciesRandomSeeded(waterMonsInfo->wildPokemon[i].species, TX_RANDOM_T_WILD_POKEMON, 0);
        ...
        if (hiddenMonsInfo != NULL) // no encounter rate check since 0 means land, 1 means water encounters
        {
            for (i = 0; i < HIDDEN_WILD_COUNT; i++)
            {
                species = GetSpeciesRandomSeeded(hiddenMonsInfo->wildPokemon[i].species, TX_RANDOM_T_WILD_POKEMON, 0);
        ...
        u16 checkSpecies;
        u8 level;
        level = DynamicEncounterLevel();
        if (landMonsInfo != NULL && landMonsInfo->encounterRate != 0)
        {
            for (i = 0; i < LAND_WILD_COUNT; i++)
            {
                species = landMonsInfo->wildPokemon[i].species;
                checkSpecies = CheckEvolve(landMonsInfo->wildPokemon[i].species, level);
                if (checkSpecies != 0)
                {
        ...
        if (waterMonsInfo != NULL && waterMonsInfo->encounterRate != 0)
        {
            ...
            {
                species = waterMonsInfo->wildPokemon[i].species;
                checkSpecies = CheckEvolve(waterMonsInfo->wildPokemon[i].species, level);
                if (checkSpecies != 0)
                {
                    species = checkSpecies;
                }
        ...
        if (hiddenMonsInfo != NULL) // no encounter rate check since 0 means land, 1 means water encounters
        {
            ...
            {
                species = hiddenMonsInfo->wildPokemon[i].species;
                checkSpecies = CheckEvolve(hiddenMonsInfo->wildPokemon[i].species, level);
                if (checkSpecies != 0)
                {
                    species = checkSpecies;
                }
...
static void TryDrawIconInSlot(u16 species, s16 x, s16 y)
{
    if (species == SPECIES_NONE || species > NUM_SPECIES)
        ...
        gSprites[CreateMonIcon(species, SpriteCB_MonIcon, x, y, 0, 0xFFFFFFFF)].oam.paletteNum = 7;
    else
...
bool8 TryFindHiddenPokemon(void)
{
    ...
    if ((*stepPtr) == 0 && (Random() % 100 < HIDDEN_MON_SEARCH_RATE))
    {
        ...
        // init search data
        gDexNavSearchDataPtr->isHiddenMon = isHiddenMon;
        gDexNavSearchDataPtr->species = species;
        gDexNavSearchDataPtr->hiddenSearch = TRUE;
        gDexNavSearchDataPtr->environment = environment;  //updated in DexNavTryGenerateMonLevel if hidden mon
        gDexNavSearchDataPtr->monLevel = DexNavTryGenerateMonLevel(species, environment);
        if (gDexNavSearchDataPtr->monLevel == MON_LEVEL_NONEXISTENT)
        {
            Free(gDexNavSearchDataPtr);
            return FALSE;
        }
        ...
        while (1) {
            if (TryStartHiddenMonFieldEffect(gDexNavSearchDataPtr->environment, 8, 8, TRUE))
                break;
            if (++attempts > 20)
        ...
        gTasks[taskId].tSpecies = gDexNavSearchDataPtr->species;
        gTasks[taskId].tEnvironment = gDexNavSearchDataPtr->environment;
...
static void DrawSearchIcon(void)
{
    ...
    spriteSheet.tag = SELECTION_CURSOR_TAG;
    LoadCompressedSpriteSheet(&spriteSheet);
    gDexNavSearchDataPtr->iconSpriteId = CreateSprite(&sSearchIconSpriteTemplate, 18, GetSearchWindowY() + 12, 0);
}
...
static void DrawHiddenSearchWindow(u8 width)
{
    ...
    AddTextPrinterParameterized3(gDexNavSearchDataPtr->windowId, 0, SPECIES_ICON_X + 4, 0, sSearchFontColor, TEXT_SKIP_DRAW, sText_ThreeQmarks);
    
    ConvertIntToDecimalStringN(gStringVar1, gDexNavSearchDataPtr->searchLevel, STR_CONV_MODE_LEFT_ALIGN, 2);
    StringExpandPlaceholders(gStringVar4, sText_SearchLevel);
    AddTextPrinterParameterized3(gDexNavSearchDataPtr->windowId, 0, SPECIES_ICON_X + 4, 12, sSearchFontColor, TEXT_SKIP_DRAW, gStringVar4);
    CopyWindowToVram(gDexNavSearchDataPtr->windowId, 2);
}
...
static void DexNavDrawHiddenIcons(void)
{
    ...
    if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT))
        gDexNavSearchDataPtr->ownedIconSpriteId = CreateSprite(&sOwnedIconTemplate, SPECIES_ICON_X + 6, GetSearchWindowY() + 2, 0);
    
    if (gDexNavSearchDataPtr->isHiddenMon)
        gDexNavSearchDataPtr->exclamationSpriteId = CreateSprite(&sHiddenMonIconTemplate, SPECIES_ICON_X + 34, GetSearchWindowY() + 8, 0);
}
