...
static void Task_NewGameBirchSpeech_ProcessResultYesNoMenu(u8);
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (gTasks[taskId].tMenuType)
        {
            ...
            case HAS_SAVED_GAME:
                ...
                {
                AddTextPrinterParameterized3(2, FONT_NORMAL, 0, 1, sTextColor_Headers, TEXT_SKIP_DRAW, gText_MainMenuContinue1);
                }
                else
                {
...
static void CB2_NewGameBirchSpeech_ReturnFromTxRandomizerChallengesOptions(void)
{
    ...
    ResetTasks();
    taskId = CreateTask(Task_NewGameBirchSpeech_Cleanup, 0);
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInWelcome(u8 taskId)
{
    ...
    {
        gSaveBlock2Ptr->version = 209; // v1.0.9
        FlagSet(FLAG_bugfix_trainer_levels);
        gSaveBlock2Ptr->Blost = 0;
        gSaveBlock1Ptr->tx_Challenges_StartLoc = START_LOC_LITTLEROOT;
        gSaveBlock1Ptr->tx_Random_Starter = 0;
        gSaveBlock2Ptr->badges = 0;
        gSaveBlock1Ptr->tx_Difficulty = 0;
        gSaveBlock2Ptr->rarecandycount = 0;
        gSaveBlock2Ptr->storedIndex = 0;
        ScriptContext_SetupScript(EventScript_clearflag);
...
static void Task_NewGameBirchSpeech_AreYouReady(u8 taskId)
{
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_ProcessResultYesNoMenu;
    }
}

static void Task_NewGameBirchSpeech_ProcessResultYesNoMenu(u8 taskId)
{
    gMain.savedCallback = CB2_NewGameBirchSpeech_ReturnFromTxRandomizerChallengesOptions;
    SetMainCallback2(CB2_InitTxRandomizerChallengesMenu);
}
...
static void MainMenu_FormatSavegameBadges(void)
{
 ...
 if (FlagGet(FLAG_OPEN_WORLD)) {
        badgeCount = gSaveBlock2Ptr->badges;
  } else {
