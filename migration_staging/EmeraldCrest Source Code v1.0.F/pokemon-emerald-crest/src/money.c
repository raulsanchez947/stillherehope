...
void PrintMoneyAmountInMoneyBox(u8 windowId, int amount, u8 speed)
{
    if (FlagGet(FLAG_SPOTLIGHT_EVENT_MENU_ACTIVE)) {
        PrintMoneyAmount(windowId, 0, 0, amount, speed);
    } else {
        PrintMoneyAmount(windowId, 38, 1, amount, speed);
    }
}
const u8 gText_Spotlight[] = _("{COLOR GREEN}{STR_VAR_2} {COLOR DARK_GRAY}Spotlight {R_BUTTON}\nEvent Happening Now!");
void PrintMoneyAmount(u8 windowId, u8 x, u8 y, int amount, u8 speed)
{
    ...
    if (FlagGet(FLAG_SPOTLIGHT_EVENT_MENU_ACTIVE)) {
        ...
        StringCopy(gStringVar2, gSpeciesNames[spotlightMon]);
        ...
        StringExpandPlaceholders(txtPtr, gText_Spotlight);
    } else {
...
void DrawMoneyBox(int amount, u8 x, u8 y)
{
    ...
    if (FlagGet(FLAG_SPOTLIGHT_EVENT_MENU_ACTIVE)) {
        SetWindowTemplateFields(&template, 0, x + 1, y + 1, 16, 4, 15, 8);
