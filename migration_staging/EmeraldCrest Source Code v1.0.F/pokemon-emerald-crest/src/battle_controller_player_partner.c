...
static void PlayerPartnerHandleChoosePokemon(void)
{
    ...
    // Switching out
    else if (gBattleStruct->monToSwitchIntoId[gActiveBattler] >= PARTY_SIZE || !IsValidForBattle(&gPlayerParty[gBattleStruct->monToSwitchIntoId[gActiveBattler]]))
    {
...
static void PlayerPartnerHandleIntroTrainerBallThrow(void)
{
    ...
    {
        LoadCompressedPalette(gTrainerFrontPicPaletteTable[gTrainers[gPartnerTrainerId].trainerPic].data, OBJ_PLTT_ID(paletteNum), PLTT_SIZE_4BPP);
    }
    else
    {
