...
{
// Ingame trade with yourself (Trade evolutions) 
    ...
    {
        struct Pokemon *pokemon = &gEnemyParty[0];
        CreateMon(pokemon, SPECIES_SHELMET, 99, 32, FALSE, 0, OT_ID_PRESET, 0); // (After the trade this pokemon is set as "Seen and caught" in the players pokedex!)
...
static void CB2_SaveAndEndTrade(void)
{
    ...
    case 50:
        if (!InUnionRoom()) {
            ...
            #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "trade +1:%d", gSaveBlock2Ptr->playerRank);
            #endif
        }
        if (gWirelessCommType)
