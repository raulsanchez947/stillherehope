...
void EnterSafariMode(void)
{
    ...
    #ifndef NDEBUG
    MgbaPrintf(MGBA_LOG_DEBUG, "entered safarizone:%d", gSaveBlock2Ptr->playerRank);
    #endif
    SetSafariZoneFlag();
    ClearAllPokeblockFeeders();
    if (ReturnSpotlightSpecies() != 0) {
        sSafariZoneStepCounter = 2550;
        gNumSafariBalls = 99;
    }
    else if (FlagGet(FLAG_GENERATED_SPOTLIGHT_EVENT))
    {
        sSafariZoneStepCounter = 2550;
        gNumSafariBalls = 99; 
    }
...
{
    if (ReturnSpotlightSpecies() && spotlightSpecies[ReturnSpotlightMonIndex()].isSpotlightMonCaptured) {
        // Disable the spotlight mon from appearing once the safari zone is over
        spotlightSpecies[ReturnSpotlightMonIndex()].spotlightToggle = FALSE;
        VarSet(VAR_UNUSED_0x40A8, 4);
    }

    FlagClear(FLAG_GENERATED_SPOTLIGHT_EVENT);
...
void CB2_EndSafariBattle(void)
{
    ...
    if (ReturnSpotlightSpecies())
        spotlightIndex = ReturnSpotlightMonIndex();
    
    sSafariZonePkblkUses += gBattleResults.pokeblockThrows;
    if (gBattleOutcome == B_OUTCOME_CAUGHT)
    {
        // This only works this way since a Spotlight mon is GUARANTEED to spawn till
        if (ReturnSpotlightSpecies())
            spotlightSpecies[spotlightIndex].isSpotlightMonCaptured = TRUE;
