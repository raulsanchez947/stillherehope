...
static const u16 sStarterGen1[STARTER_MON_COUNT] =
{
    SPECIES_SQUIRTLE,
    SPECIES_BULBASAUR,
    SPECIES_CHARMANDER,
...
static const u16 sStarterGen2[STARTER_MON_COUNT] =
{
    SPECIES_TOTODILE,
    SPECIES_CHIKORITA,
    SPECIES_CYNDAQUIL,
...
static const u16 sStarterGen3[STARTER_MON_COUNT] =
{
    SPECIES_TREECKO,
    SPECIES_TORCHIC,
    SPECIES_MUDKIP,
...
static const u16 sStarterGen4[STARTER_MON_COUNT] =
{
    SPECIES_PIPLUP,
    SPECIES_TURTWIG,
    SPECIES_CHIMCHAR,
...
static const u16 sStarterGen5[STARTER_MON_COUNT] =
{
    SPECIES_OSHAWOTT,
    SPECIES_SNIVY,
    SPECIES_TEPIG,
...
static const u16 sStarterGen6[STARTER_MON_COUNT] =
{
    SPECIES_FROAKIE,
    SPECIES_CHESPIN,
    SPECIES_FENNEKIN,
...
static const u16 sStarterGen7[STARTER_MON_COUNT] =
{
    SPECIES_POPPLIO,
    SPECIES_ROWLET,
    SPECIES_LITTEN,
...
static const u16 sStarterGen8[STARTER_MON_COUNT] =
{
    SPECIES_SOBBLE,
    SPECIES_GROOKEY,
    SPECIES_SCORBUNNY,
...
static const u16 sStarterGen9[STARTER_MON_COUNT] =
{
    SPECIES_SPRIGATITO,
    SPECIES_FUECOCO,
    SPECIES_QUAXLY,
...
u16 GetStarterPokemon(u16 chosenStarterId)
{
    ...
    else if (gSaveBlock1Ptr->tx_Random_Starter == 0)
    {
        mon = sStarterMon[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 1)
    {
        if (sStarterList[chosenStarterId] == 0)
        sStarterList[chosenStarterId] = PickRandomStarter(sStarterList, chosenStarterId);
        mon = sStarterList[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 2)
    {
        mon = sStarterGen1[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 3)
    {
        mon = sStarterGen2[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 4)
    {
        mon = sStarterGen3[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 5)
    {
        mon = sStarterGen4[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 6)
    {
        mon = sStarterGen5[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 7)
    {
        mon = sStarterGen6[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 8)
    {
        mon = sStarterGen7[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 9)
    {
        mon = sStarterGen8[chosenStarterId];
    }
    else if (gSaveBlock1Ptr->tx_Random_Starter == 10)
    {
        mon = sStarterGen9[chosenStarterId];
    }

    #ifndef NDEBUG
        MgbaPrintf(MGBA_LOG_DEBUG, "new species[%d]", mon);
    #endif
    if (VarGet(VAR_TEMP_5) == chosenStarterId) 
        VarSet(VAR_TEMP_6, mon);
    return mon;
}
...
void CB2_ChooseStarter(void)
{
    u8 taskId;
    u8 spriteId;
    u8 sel = Random() % STARTER_MON_COUNT;

    FlagClear(FLAG_SHINY_STARTER);
    VarSet(VAR_TEMP_7, 1);
    VarSet(VAR_TEMP_5, sel);
...
static void Task_StarterChoose(u8 taskId)
{
    ...
    AddTextPrinterParameterized(0, FONT_NORMAL, gText_BirchInOP, 0, 1, 0, NULL);
    }
    else{
...
static u8 CreatePokemonFrontSprite(u16 species, u8 x, u8 y)
{
    ...
    if (VarGet(VAR_TEMP_7) == 1)
    {
        if (VarGet(VAR_TEMP_6) == species)
        {
            FlagSet(FLAG_SHINY_STARTER);
            spriteId = CreateMonPicSprite_Affine(species, 0, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
