...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    bool8 hasCutAlready = FALSE;
    bool8 hasRockSmashAlready = FALSE;
    bool8 hasStrengthAlready = FALSE;
    bool8 hasSurfAlready = FALSE;
    bool8 hasDiveAlready = FALSE;
    bool8 hasWaterfallAlready = FALSE;
        ...
        for (j = 0; sFieldMoves[j] != FIELD_MOVES_COUNT; j++)
        {
            if (GetMonData(&mons[slotId], i + MON_DATA_MOVE1) == sFieldMoves[j])
            {
                ...
                case FIELD_MOVE_ROCK_SMASH:
                    hasRockSmashAlready = TRUE;
                    break;
                case FIELD_MOVE_STRENGTH:
                    hasStrengthAlready = TRUE;
                    break;
    ...
    // Check if the selected Pokémon can learn Fly
    if (FlagGet(FLAG_OPEN_WORLD))
    {
        if (CanTeachMove(&mons[slotId], MOVE_FLASH) != ALREADY_KNOWS_MOVE && CanTeachMove(&mons[slotId], MOVE_FLASH) == CAN_LEARN_MOVE) {
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_FIELD_MOVES + FIELD_MOVE_FLASH);
            hasFlashAlready = TRUE;
        }
        
        if (CanTeachMove(&mons[slotId], MOVE_FLY) != ALREADY_KNOWS_MOVE && CanTeachMove(&mons[slotId], MOVE_FLY) == CAN_LEARN_MOVE)
        {
    ...
    {
        if (CanTeachMove(&mons[slotId], MOVE_FLASH) != ALREADY_KNOWS_MOVE && CanTeachMove(&mons[slotId], MOVE_FLASH) == CAN_LEARN_MOVE && CheckBagHasItem(ITEM_HM05_FLASH, 1)) {
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_FIELD_MOVES + FIELD_MOVE_FLASH);
            hasFlashAlready = TRUE;
        }
        
        if (CanTeachMove(&mons[slotId], MOVE_FLY) != ALREADY_KNOWS_MOVE && CanTeachMove(&mons[slotId], MOVE_FLY) == CAN_LEARN_MOVE && CheckBagHasItem(ITEM_HM02_FLY, 1))
        {
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_FIELD_MOVES + FIELD_MOVE_FLY);
            hasFlyAlready = TRUE;
        }
    ...
    {
        if (hasCutAlready == FALSE && CheckBagHasItem(ITEM_HM01_CUT, 1))
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_FIELD_MOVES + FIELD_MOVE_CUT);
        if (hasFlashAlready == FALSE && CheckBagHasItem(ITEM_HM05_FLASH, 1))
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 1 + MENU_FIELD_MOVES);
        if (hasRockSmashAlready == FALSE && CheckBagHasItem(ITEM_HM06_ROCK_SMASH, 1))
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_FIELD_MOVES + FIELD_MOVE_ROCK_SMASH);
        if (hasStrengthAlready == FALSE && CheckBagHasItem(ITEM_HM04_STRENGTH, 1))
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_FIELD_MOVES + FIELD_MOVE_STRENGTH);
        if (hasSurfAlready == FALSE && CheckBagHasItem(ITEM_HM03_SURF, 1))
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_FIELD_MOVES + FIELD_MOVE_SURF);
        if (hasFlyAlready == FALSE && CheckBagHasItem(ITEM_HM02_FLY, 1))
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
        if (hasDiveAlready == FALSE && CheckBagHasItem(ITEM_HM08_DIVE, 1))
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_FIELD_MOVES + FIELD_MOVE_DIVE);
        if (hasWaterfallAlready == FALSE && CheckBagHasItem(ITEM_HM07_WATERFALL, 1))
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_FIELD_MOVES + FIELD_MOVE_WATERFALL);
    }
...
void CB2_ShowPartyMenuForItemUse(void)
{
    ...
    if (GetItemEffectType(gSpecialVar_ItemId) == ITEM_EFFECT_SACRED_ASH
     && GetPocketByItemId(gSpecialVar_ItemId) != POCKET_KEY_ITEMS)
    {
        gPartyMenu.slotId = 0;
        for (i = 0; i < PARTY_SIZE; i++)
        {
...
static void Task_LearnedMove(u8 taskId)
{
    ...
    if (move[1] == 0)
    {
        ...
        // Uncomment to disable reusable TMs
...
static void CB2_ReturnToPartyMenuWhileLearningMove(void)
{
    ...
    if (gSpecialVar_ItemId >= ITEM_RARE_CANDY && gSpecialVar_ItemId <= ITEM_DYNAMAX_CANDY && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(gSpecialVar_ItemId, 1))
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    u32 exp, currLvlExp, expToNextLvl;
    u16 species;

    sInitialLevel = GetMonData(mon, MON_DATA_LEVEL);
    species = GetMonData(mon, MON_DATA_SPECIES);
    exp = GetMonData(mon, MON_DATA_EXP, NULL);
    currLvlExp = gExperienceTables[gSpeciesInfo[species].growthRate][sInitialLevel];
    expToNextLvl;

    exp -= currLvlExp;
    expToNextLvl = gExperienceTables[gSpeciesInfo[species].growthRate][sInitialLevel + 1] - currLvlExp;
    if (sInitialLevel < GetCurrentPartyLevelCap())
    {
        if (gSaveBlock1Ptr->tx_Challenges_LevelCap != 0 && holdEffectParam != 0) {
            if (expToNextLvl <= sExpCandyExperienceTable[holdEffectParam - 1]) {
    ...
    else
    {
        ...
        if (sFinalLevel > sInitialLevel)
        {
            ...
            {
                gSaveBlock2Ptr->rarecandycount += 1;
                if (gSaveBlock2Ptr->rarecandycount >= 50)
                {
...
void ItemUseCB_SacredAsh(u8 taskId, TaskFunc task)
{
    ...
    u32 cap = min(GetCurrentPartyLevelCap(), MAX_LEVEL);
    u32 exp;
    ...
    StringExpandPlaceholders(gStringVar4, sText_HolyBluntUsed);
    DisplayPartyMenuMessage(gStringVar4, TRUE);
    SetPartyMonAilmentGfx(mon, &sPartyMenuBoxes[gPartyMenu.slotId]);
    DisplayPartyPokemonLevelCheck(mon, &sPartyMenuBoxes[gPartyMenu.slotId], 1);
