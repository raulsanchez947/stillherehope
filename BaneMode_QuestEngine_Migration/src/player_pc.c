...
static void PlayerPC_WonderTrade(u8);
static void PlayerPC_About(u8);
...
//    [MENU_MAILBOX]     = { gText_Mailbox,     PlayerPC_Mailbox },
    ...
    [MENU_WONDERTRADE] = { gText_WonderTrade, PlayerPC_WonderTrade},
    [MENU_ABOUT]       = {gText_About,        PlayerPC_About}
...
static void PlayerPC_WonderTrade(u8 taskId)
{
    ScriptContext_SetupScript(EventScript_CheckDewfordBadge);
    DestroyTask(taskId);
}

static void PlayerPC_About(u8 taskId)
{  
   if (gSaveBlock2Ptr->version == 192)
     {
       ScriptContext_SetupScript(EventScript_UpdateMessage);
    //    ReshowPlayerPC(taskId);
        gSaveBlock2Ptr->version = 193;
        FlagClear(FLAG_LOCK_BOX);
     }
     else
     {
       ScriptContext_SetupScript(EventScript_AboutMessage);
    //    ReshowPlayerPC(taskId);
     }
     DestroyTask(taskId);
}
