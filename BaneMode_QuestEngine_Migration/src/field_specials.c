...
{
    if (FlagGet(FLAG_OPEN_WORLD) && FlagGet(FLAG_BADGE02_GET) && FlagGet(FLAG_BADGE01_GET) && !FlagGet(FLAG_ENABLE_ROXANNE_FIRST_CALL_OPW)) {
        VarSet(VAR_ROXANNE_CALL_STEP_COUNTER, 0);
        FlagSet(FLAG_ENABLE_ROXANNE_FIRST_CALL);
    }
...
void IsGrassTypeInParty(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (GetMonData(pokemon, MON_DATA_SANITY_HAS_SPECIES) && !GetMonData(pokemon, MON_DATA_IS_EGG))
        {
            ...
            if (GetTypeBySpecies(species, 1) == TYPE_GRASS || GetTypeBySpecies(species, 2) == TYPE_GRASS) 
            {
...
void GiveLeadMonEffortRibbon(void)
{
    ...
    #ifndef NDEBUG
    MgbaPrintf(MGBA_LOG_DEBUG, "ribbon recevied +5:%d", gSaveBlock2Ptr->playerRank);
    #endif
...
u16 GetDaysUntilSeasonChange(void)
{
    u8 option = 2;
    u16 seasonChangeDay = VarGet(VAR_SEASON_CHANGE_DAY);

    if (gSaveBlock2Ptr->seasons == 1)
        option = 7;
    
    if (gLocalTime.days - seasonChangeDay >= option)
        return 0;
    else if (gLocalTime.days < 0)
        return option + 1;

    return option - (gLocalTime.days - seasonChangeDay);
}

u16 SetSeasonChangeDay(void)
{
    VarSet(VAR_SEASON_CHANGE_DAY, gLocalTime.days);
    return gLocalTime.days;
}
...
void BaneModeSetSelectedMonNature(void)
{
    if (gSpecialVar_0x8004 >= PARTY_SIZE || gSpecialVar_0x8005 >= NUM_NATURES)
    {
        gSpecialVar_Result = FALSE;
        return;
    }

    SetNature(&gPlayerParty[gSpecialVar_0x8004], gSpecialVar_0x8005);
    CalculateMonStats(&gPlayerParty[gSpecialVar_0x8004]);
    gSpecialVar_Result = TRUE;
}

void BaneModeGiveAllTMs(void)
{
    u16 itemId;
...
static void BaneModeGiveBagOrPcItem(u16 itemId, u16 quantity)
{
    if (!AddBagItem(itemId, quantity))
        AddPCItem(itemId, quantity);
}

void BaneModeGiveLegacyBirchBundle(void)
{
    u16 itemId;
    ...
    for (itemId = ITEM_VENUSAURITE; itemId <= ITEM_DIANCITE; itemId++)
        BaneModeGiveBagOrPcItem(itemId, 1);
