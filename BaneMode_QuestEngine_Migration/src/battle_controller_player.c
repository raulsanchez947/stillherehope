...
static void MoveSelectionRemoveMoveSelector(void);
...
EWRAM_DATA u8 sUseDefaultValueInPlaceofgMultiplayerCursor = FALSE;  // Bad name :p. Its meant to make the moves not display any type effectiveness in doubles until the user is selecting a target
...
static void HandleInputChooseTarget(void)
{
    ...
    if (!sUseDefaultValueInPlaceofgMultiplayerCursor && gSaveBlock1Ptr->in_battle_type_effectiveness && !gBattleStruct->zmove.viewing)
    {
    ...
    if (JOY_NEW(A_BUTTON))
    {
        ...
        if (gBattleStruct->mega.playerSelect)
            ...
            BtlController_EmitTwoReturnValues(BUFFER_B, 10, gMoveSelectionCursor[gActiveBattler] | RET_ULTRA_BURST | (gMultiUsePlayerCursor << 8));
        else
    ...
    {
        PlaySE(SE_SELECT);
        // Return the move's text to default once the user backs out of choosing a target
        if (gSaveBlock1Ptr->in_battle_type_effectiveness && !gBattleStruct->zmove.viewing)
        {
            sUseDefaultValueInPlaceofgMultiplayerCursor = TRUE;
            MoveSelectionDisplayMoveNames();
        }
        gSprites[gBattlerSpriteIds[gMultiUsePlayerCursor]].callback = SpriteCB_HideAsMoveTarget;
    ...
    else if (JOY_NEW(DPAD_LEFT | DPAD_UP))
    {
        ...
        if (gSaveBlock1Ptr->in_battle_type_effectiveness && !gBattleStruct->zmove.viewing)
            // Refresh when switching between targets using DPAD_LEFT or DPAD_UP
            MoveSelectionDisplayMoveNames();
        else if (gBattleStruct->zmove.viewing)
            // Recheck the type effectiveness of the zmove
            MoveSelectionDisplayZMove(gBattleStruct->zmove.chosenZMove, Z_MOVE_REFRESH_MOVE);
...
static void HandleInputShowEntireFieldTargets(void)
{
    ...
    if (!sUseDefaultValueInPlaceofgMultiplayerCursor && gSaveBlock1Ptr->in_battle_type_effectiveness)
    {
        MoveSelectionDisplayMoveNames();
        sUseDefaultValueInPlaceofgMultiplayerCursor = TRUE;
    }
...
static void HandleInputChooseMove(void)
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        ...
        default:
            ...
            break;
        case 1:
            sUseDefaultValueInPlaceofgMultiplayerCursor = FALSE;      // Making sure it is cleared when a target is to be selected
            gBattlerControllerFuncs[gActiveBattler] = HandleInputChooseTarget;
    ...
    else if (JOY_NEW(B_BUTTON) || gPlayerDpadHoldFrames > 59)
    {
        ...
        {
            sUseDefaultValueInPlaceofgMultiplayerCursor = TRUE; // To prevent a previously loaded move never loads with color
    ...
    else if (JOY_NEW(DPAD_LEFT) && !gBattleStruct->zmove.viewing)
    {
        if (gMoveSelectionCursor[gActiveBattler] & 1)
        {
            ...
            PlaySE(SE_SELECT);
            if (gSaveBlock1Ptr->in_battle_type_effectiveness)
            {
                sUseDefaultValueInPlaceofgMultiplayerCursor = TRUE;
                MoveSelectionDisplayMoveNames();
            }
    ...
    else if (JOY_NEW(START_BUTTON))
    {
        ...
        else if (gBattleStruct->zmove.viable)
        {
            ...
            PlaySE(SE_SELECT);
            if (!gBattleStruct->zmove.viewing)
                MoveSelectionDisplayZMove(gBattleStruct->zmove.chosenZMove, Z_MOVE_LOAD_MOVE);
            else
                ReloadMoveNames();
        }
    }
...
static void HandleMoveSwitching(void)
{
    ...
    if (JOY_NEW(A_BUTTON | SELECT_BUTTON))
    {
        ...
        if (gMoveSelectionCursor[gActiveBattler] != gMultiUsePlayerCursor)
        {
            ...
            MoveSelectionDisplayMoveNames();
            MoveSelectionRemoveMoveSelector();
...
static void MoveSelectionDisplayMoveNames(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        // Decides the type effectiveness of a move and shows it
        if (i == gMoveSelectionCursor[gActiveBattler] && gSaveBlock1Ptr->in_battle_type_effectiveness)
        {
            AddTypeEffectivenessAndMoveNameToString(gDisplayedStringBattle, gMoveNames[moveInfo->moves[i]]);
        }
        else
...
// Removed from MoveSelectionDisplayMoveNames and put in its own function
// Reason: There is no good reason to refresh the move selector arrow EVERY SINGLE TIME in the above mentioned function is called
// and since I'm using this in a lot more places, it gets annoying.
static void MoveSelectionRemoveMoveSelector(void)
{
    u8 i;
                                                                      ...
                                                                      and returns the appropriate value of the struct in the struct array gTextOnWindowsInfo_Normal */
{
	...
	else if (gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].effect == EFFECT_LEVEL_DAMAGE 
    || gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].effect == EFFECT_COUNTER 
    || gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].effect == EFFECT_MIRROR_COAT
    || gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].target == MOVE_TARGET_RANDOM
    || gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].target == MOVE_TARGET_DEPENDS)
    ...
    || gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].effect == EFFECT_COUNTER 
    || gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].effect == EFFECT_MIRROR_COAT 
    || gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].effect == EFFECT_METAL_BURST    
    || gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].target == MOVE_TARGET_RANDOM
    || gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].target == MOVE_TARGET_DEPENDS) 
    {
        return B_WIN_MOVE_TYPE;
...
{
	u8 *txtPtr;
	struct ChooseMoveStruct *moveInfo = (struct ChooseMoveStruct*)(&gBattleResources->bufferA[gActiveBattler][MAX_BATTLERS_COUNT]); // &gBattleBufferA should be &gBattleResources->bufferA if it doesn't compile with the former
...
static void PlayerHandleChooseMove(void)
{
    ...
    else
    {
        ...
        if (CanUltraBurst(gActiveBattler))
            CreateBurstTriggerSprite(gActiveBattler, 0);
...
void AddTypeEffectivenessAndMoveNameToString(u8 *string, const u8 *moveName)
{
    u8 j, moveId;
    u8 moveEffectiveness, movEff2;          // Stores an index to gTextOnWindowsInfo_Normal that gets the appropriate window Id properties
    u8 *txtPtr = string;                    // Create an alternate pointer to modify the string
    u8 typeEffPriority[4] = {B_WIN_SUPER_EFFECTIVE, B_WIN_NO_EFFECT, B_WIN_NOT_VERY_EFFECTIVE, B_WIN_MOVE_TYPE};
    struct ChooseMoveStruct *moveInfo = (struct ChooseMoveStruct *)(&gBattleResources->bufferA[gActiveBattler][4]);

    // Get the id of the move in the user's moveset using the move's name
    for (moveId = 0; moveId < MAX_MON_MOVES; moveId++)
    {
        if (StringCompare(gMoveNames[moveInfo->moves[moveId]], moveName) == 0 && gMoveNames[moveInfo->moves[moveId]] != MOVE_NONE)
            break;
    }

    // Get the type effectiveness of the target
    if (!IsDoubleBattle())
    {   // For all single battles
        moveEffectiveness = TypeEffectiveness(moveInfo, B_POSITION_OPPONENT_LEFT);
    }
    else // This should work for all double/multi* battles
    {
        // The type effectiveness should only be displayed when the user decides to choose a target (excluding multi-target moves)
        if (sUseDefaultValueInPlaceofgMultiplayerCursor)
        {
            moveEffectiveness = B_WIN_MOVE_TYPE;
            sUseDefaultValueInPlaceofgMultiplayerCursor = FALSE;
        }
        else
        {
        ...
        // For stuff like Surf, Earthquake, Razor Leaf etc. Note that it will not detect the effectiveness to allies
        if (gBattleMoves[moveInfo->moves[moveId]].target == MOVE_TARGET_BOTH
            || gBattleMoves[moveInfo->moves[moveId]].target == MOVE_TARGET_FOES_AND_ALLY)
        {
            moveEffectiveness = TypeEffectiveness(moveInfo, B_POSITION_OPPONENT_LEFT);
            movEff2 = TypeEffectiveness(moveInfo, B_POSITION_OPPONENT_RIGHT);

            // Prioritize Effectiveness as Super Eff > No Eff > Not Very Eff > Normal Effectiveness
            for (j = 0; j < ARRAY_COUNT(typeEffPriority); j++)
            {
                if (moveEffectiveness == typeEffPriority[j] || movEff2 == typeEffPriority[j])
                    moveEffectiveness = typeEffPriority[j];
            }
        }
    }

    // Set colors using txtPtr
    *(txtPtr)++ = EXT_CTRL_CODE_BEGIN;
    ...
    *(txtPtr)++ = gTextOnWindowsInfo_Normal[moveEffectiveness].shadowColor;
    ...
    // Add move name
    StringCopy(txtPtr, moveName);

    // Max length of string before it starts chopping off.
    if (GetStringWidth(FONT_NORMAL, moveName, 0) <= 63)
    {
