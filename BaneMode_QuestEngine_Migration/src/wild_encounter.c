...
u8 ChooseWildMonLevel(const struct WildPokemon *wildPokemon, u8 wildMonIndex, u8 area)
{
    ...
    if (FlagGet(FLAG_OPEN_WORLD) && wildHeader->mapNum != MAP_NUM(ROUTE130))
    {
...
bool8 SpotlightChecks(void)
{
    if (FlagGet(FLAG_IS_SPOTLIGHT_EVENT) && GetSafariZoneFlag())
    {
        return TRUE;
    } else if (FlagGet(FLAG_GENERATED_SPOTLIGHT_EVENT) && GetSafariZoneFlag())
    {
        return TRUE;
    }
...
void CreateWildMon(u16 species, u8 level)
{
    ...
    u16 checkSpecies;
    u8 seasonalEncounters = Random() % 30;
    // if (gSaveBlock2Ptr->seasonalEncounters) {
    //     MgbaPrintf(MGBA_LOG_DEBUG, "seasonalEncounters chance: %d", seasonalEncounters);
    // }
    ...
    if (gSaveBlock1Ptr->tx_Random_WildPokemon && !SpotlightChecks()) 
    {
        #ifndef NDEBUG
    ...
    else if (FlagGet(FLAG_OPEN_WORLD) && !SpotlightChecks())
    {
        
        if (seasonalEncounters == 0 && FlagGet(FLAG_SEASONS) && gSaveBlock2Ptr->seasonalEncounters)
        {
            bool8 validSpecies = FALSE;
            if (gSpeciesInfo[species].types[0] == TYPE_ICE || gSpeciesInfo[species].types[1] == TYPE_ICE)
                validSpecies = TRUE;
            else if (gSpeciesInfo[species].types[0] == TYPE_WATER || gSpeciesInfo[species].types[1] == TYPE_WATER)
                validSpecies = TRUE;
            ...
            while (!validSpecies)
            {
                ...
                if (species != SPECIES_NONE || species != SPECIES_EGG) {
                    ...
                    case SEASON_SUMMER:
                        ...
                        break;
                    case SEASON_FALL:
                        if (gSpeciesInfo[species].bodyColor == BODY_COLOR_RED || gSpeciesInfo[species].eggGroups[0] == EGG_GROUP_AMORPHOUS || gSpeciesInfo[species].eggGroups[1] == EGG_GROUP_AMORPHOUS)
                            validSpecies = TRUE;
                        break;
                    case SEASON_WINTER:
                        if (gSpeciesInfo[species].types[0] == TYPE_ICE || gSpeciesInfo[species].types[1] == TYPE_ICE)
                        ...
                        break;
                    case SEASON_SPRING:
                        if (gSpeciesInfo[species].bodyColor == BODY_COLOR_PINK || gSpeciesInfo[species].types[0] == TYPE_FAIRY || gSpeciesInfo[species].types[1] == TYPE_FAIRY)
                            validSpecies = TRUE;
                        break;
                    default:
                        validSpecies = TRUE;
                    ...
                    if ((gSpeciesInfo[species].flags & (SPECIES_FLAG_LEGENDARY | SPECIES_FLAG_MYTHICAL | SPECIES_FLAG_MEGA_EVOLUTION | SPECIES_FLAG_ULTRA_BEAST | SPECIES_FLAG_ULTRA_BURST | SPECIES_FLAG_PRIMAL_REVERSION | SPECIES_FLAG_PARADOX | SPECIES_FLAG_CANNOT_BE_TRADED))  || FlagGet(FLAG_ROAMING_MON)) {
                        validSpecies = FALSE;
                    }
                }
                else
        ...
        checkSpecies = CheckEvolve(species, level);
        if (checkSpecies != 0)
        {
            species = checkSpecies;
        }
    } else if (FlagGet(FLAG_SEASONS) && gSaveBlock2Ptr->seasonalEncounters && !SpotlightChecks())
    {
        if (seasonalEncounters == 0)
        {
            bool8 validSpecies = FALSE;
            if (gSpeciesInfo[species].types[0] == TYPE_ICE || gSpeciesInfo[species].types[1] == TYPE_ICE)
                validSpecies = TRUE;
        ...
        && Random() % 3 != 0)
    {
        ...
        CreateMonWithGenderNatureLetter(&gEnemyParty[0], species, level, MAX_PER_STAT_IVS, gender, PickWildMonNature(), 0);
        return;
    }
...
void CreateDexnavWildMon(u16 species, u8 level)
{
    bool32 checkCuteCharm;
...
bool8 TryGenerateWildMon(const struct WildPokemonInfo *wildMonInfo, u8 area, u8 flags)
{
    u8 wildMonIndex = 0;
    u8 level;
    u16 spotlightMon = ReturnSpotlightSpecies();
    ...
    if (FlagGet(FLAG_GENERATED_SPOTLIGHT_EVENT)) {
        spotlightMon = gSpecialVar_Unused_0x8014;
    }
    // So on particular days in the Safari Zone, one mon will populate the entire zone.
    // Only spawn on land.
    if ((spotlightMon != SPECIES_NONE) && (area == WILD_AREA_LAND) && GetSafariZoneFlag())
    {
        u8 rand;
        rand = Random() % 100;
        ...
        level = Random() % 9 + 2; 
        CreateWildMon(spotlightMon, level);
    }
    else
...
void FishingWildEncounter(u8 rod)
{
    ...
    gSaveBlock2Ptr->playerRank += 2;
    #ifndef NDEBUG
    MgbaPrintf(MGBA_LOG_DEBUG, "fishing +2:%d", gSaveBlock2Ptr->playerRank);
    #endif
...
bool8 TryDoDoubleWildBattle(void)
{
    ...
    if (NuzlockeIsCaptureBlocked == FALSE) {
        return FALSE;
    }
