...
{
    FlagClear(FLAG_LOCK_BOX);
    FlagClear(FLAG_NO_ESCAPE);
    gSaveBlock2Ptr->Blost = (gSaveBlock2Ptr->Blost += 1);
    if (IsNuzlockeActive()) 
    {
...
static void ApplyBaneModeRouteFieldPressure(void)
{
    u32 seed;
    u16 terrain;
    ...
    seed = gMapHeader.regionMapSectionId + gSaveBlock1Ptr->location.mapGroup + gSaveBlock1Ptr->location.mapNum;
...
void LoadMapFromCameraTransition(u8 mapGroup, u8 mapNum)
{
    ...
    ApplyBaneModeRouteFieldPressure();
    ChooseAmbientCrySpecies();
...
void UpdateAltBgPalettes(u16 palettes) {
    ...
    if (gMapHeader.regionMapSectionId == MAPSEC_SLATEPORT_CITY || gMapHeader.regionMapSectionId == MAPSEC_ROUTE_109) {
...
void CB2_NewGame(void)
{
    ...
    if (gSaveBlock1Ptr->tx_Challenges_OpenWorld == 1)
    {
        gFieldCallback = NULL;
    }
    else
...
void CB2_WhiteOut(void)
{
    ...
    if (++gMain.state >= 120)
    {
        ...
        if (gSaveBlock1Ptr->tx_Challenges_NuzlockeHardcore && !FlagGet(FLAG_IS_CHAMPION)) 
        {
            ClearSaveData();
            DoSoftReset();
        }
...
static void FollowingPokemonReturnToField(void)
{
    u16 species;
    u8 form; 
    bool8 shiny;
