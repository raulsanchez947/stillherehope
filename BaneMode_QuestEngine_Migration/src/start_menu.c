...
enum
{
    ...
    MENU_ACTION_POKENAV,
    MENU_ACTION_BAG,
    MENU_ACTION_BOX, // PSS IN THE START MENU
    MENU_ACTION_PLAYER,
...
static EWRAM_DATA u8 sStartMenuPage;                          // This stores the page the start menu is currently on
static EWRAM_DATA u8 sStartMenuLastPage;                     // This stores the number of the possible amount of start menu pages (starting at 0). Not actually useful since it just represents (NUM_OF_START_MENUS - 1)
static EWRAM_DATA u8 sCurrentPageOptionCounter;               // This keeps track on the amount of options that get loaded into a page
...
static bool8 StartMenuPCCallback(void); // PSS IN THE START MENU
...
// gWindows[GetStartMenuWindowId()].window.tilemapLeft is the distance of the icon from the left border of the start menu.
// gWindows[GetStartMenuWindowId()].window.tilemapTop is the distance of the icon from the top border of the start menu.
...
#define spritePaletteTagId 0x8654
...
static const struct SpriteFrameImage sStartMenuIconFrames[] = {
    ...
    overworld_frame(sStartMenuIconsGfx, 4, 4, 16),
    overworld_frame(sStartMenuIconsGfx, 4, 4, 17),
    overworld_frame(sStartMenuIconsGfx, 4, 4, 18),
    overworld_frame(sStartMenuIconsGfx, 4, 4, 19),
    overworld_frame(sStartMenuIconsGfx, 4, 4, 20),
    overworld_frame(sStartMenuIconsGfx, 4, 4, 21),
};
...
static const struct MenuAction sStartMenuItems[] =
{
    ...
    [MENU_ACTION_BOX]             = {gText_MenuBox,      {.u8_void = StartMenuPCCallback}}, // PSS IN THE START MENU
...
static void BuildStartMenuActions(void)
{
    ...
    else
    {
        ...
        ReturnSpotlightSpecies();
        if (FlagGet(FLAG_IS_SPOTLIGHT_EVENT) && FlagGet(FLAG_ADVENTURE_STARTED)) {
            FlagSet(FLAG_SPOTLIGHT_EVENT_MENU_ACTIVE);
            DrawMoneyBox(0, 0, 0);
        }
    #endif
    }
...
{
    u8 i;
    FlagSet(FLAG_IS_MULTI_START_MENU_ENABLED); 
    FlagClear(OW_FLAG_NO_ENCOUNTER);
    FlagClear(FLAG_NO_ESCAPE); 
    if (gSaveBlock2Ptr->version == 88)
    {
        ClearSaveData();
        DoSoftReset();
        return;
    }
    if (FlagGet(FLAG_ADVENTURE_STARTED) == FALSE && FlagGet(FLAG_SYS_POKEDEX_GET) == TRUE) { //BUGFIX-V1.0.9
        FlagSet(FLAG_ADVENTURE_STARTED);
    }
    BadgeCount();

    i = (NUM_OF_START_MENU_OPTIONS * sStartMenuPage);
    sStartMenuLastPage = NUM_OF_START_MENUS - 1;

    for (i; i < (NUM_OF_START_MENU_OPTIONS * (sStartMenuPage + 1)); i++)
    {
        // To make sure it doesn't crash after loading all intended start menu options
        if (i <= FINAL_START_MENU_OPTION) 
        {
            // Any option that has a condition to show should be added here
            if((!FlagGet(FLAG_SYS_POKEDEX_GET) && i == MENU_ACTION_POKEDEX)
               || (!FlagGet(FLAG_SYS_POKEMON_GET) && (i == MENU_ACTION_POKEMON || i == MENU_ACTION_BOX))
               || (!FlagGet(FLAG_SYS_POKENAV_GET) && i == MENU_ACTION_POKENAV)
               || (!FlagGet(FLAG_SYS_DEXNAV_GET) && i == MENU_ACTION_DEXNAV)
               || (!FlagGet(FLAG_SYS_QUEST_MENU_GET) && i == MENU_ACTION_QUEST_MENU)
               // Don't build the Exit action here
               || (i == MENU_ACTION_EXIT)
               // Skip the safari and battle frontier options. You can remove this check if you want
               || (i >= MENU_ACTION_RETIRE_SAFARI && i <= MENU_ACTION_PYRAMID_BAG))
            {  
                /*Skip*/;
            }
            else 
            {
                // No other check required so proceed to print as normal
                AddStartMenuAction(i);
            }
        }
    }
    // Print the Exit Option at the end
    ...
    do
    {
        ...
        else 
        {
            if (IS_MULTI_START_MENU_ENABLED && (NUM_OF_START_MENUS > 1)) //Makes sure the the multi start menu is enabled AND there isn't only 1 page before trying to print out the appropiate text for the Exit option
            {
                ...
                {
                    if (sStartMenuPage != sStartMenuLastPage) 
...
void ShowStartMenu(void)
{
    ...
    // If for some reason, the icons are already on screen when this function is called, this will clean everything up
    if (gShouldStartMenuIconsBePrinted)
    {
        HideStartMenuWindow();
    ...
    {
        // End the dexnav to prevent unforseen bugs
        EndDexNavSearch(gDexNavSearchDataPtr->taskId);
    }
...
static bool8 HandleStartMenuInput(void)
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        ...
        if (FlagGet(FLAG_SPOTLIGHT_EVENT_MENU_ACTIVE)) {
            HideMoneyBox();
            FlagClear(FLAG_SPOTLIGHT_EVENT_MENU_ACTIVE);
        }
        return FALSE;
    }
    ...
    if (FlagGet(FLAG_SPOTLIGHT_EVENT_MENU_ACTIVE)) {
        if (JOY_NEW(R_BUTTON))
        {
            ...
            RemoveExtraStartMenuWindows();
            HideStartMenu();
            ScriptContext_SetupScript(EventScript_Spotlight);
            FlagClear(FLAG_SPOTLIGHT_EVENT_MENU_ACTIVE);
            return TRUE;
        }
    }

    if (FlagGet(FLAG_OPEN_WORLD) && IsMapRandmoized() == TRUE)
    {
        if (JOY_NEW(L_BUTTON)){
            HideMoneyBox();
            RemoveExtraStartMenuWindows();
            HideStartMenu();
            ScriptContext_SetupScript(EventScript_Softlock);
            return TRUE;
        }
    }

    // Must be enabled to use the multi menu and there is more than just 1 menu
    if (IS_MULTI_START_MENU_ENABLED && NUM_OF_START_MENUS != 1)
    {
        // Scrolls to the next start menu page
        if (JOY_NEW(DPAD_RIGHT))
        {
            ...
            if (sStartMenuPage < sStartMenuLastPage)
                ...
                // If at the last page, it will jump back to the first one
                sStartMenuPage = 0;
        ...
        // Scrolls to the previous start menu page
        if (JOY_NEW(DPAD_LEFT))
        {
            ...
            if(sStartMenuPage > 0)
                ...
                //If at the first page, it will jump to the last one
                sStartMenuPage = sStartMenuLastPage;
...
bool8 StartMenuPokedexCallback(void)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        #ifndef NDEBUG
        MgbaPrintf(MGBA_LOG_DEBUG, "pokedex used +1:%d", gSaveBlock2Ptr->playerRank);
        #endif
...
static bool8 StartMenuPCCallback(void)
{
        ...
        RemoveExtraStartMenuWindows();
        HideStartMenu();
		ScriptContext_SetupScript(EventScript_Box);
...
static void ShowSaveInfoWindow(void)
{
    ...
    // Print game mode
    yOffset += 16;
    AddTextPrinterParameterized(sSaveInfoWindowId, FONT_NORMAL, gText_SavingMode, 0, yOffset, TEXT_SKIP_DRAW, NULL);
...
static bool8 StartMenuDexNavCallback(void)
{
    // Reloads the right palette to prevent the dexnav from bugging the start menu icons
    sIsStartMenuIconPaletteLoaded = FALSE;
...
void LoadStartMenuIcon(u8 iconId, u8 position)
{
    ...
    if (flashLevel)
    {
        // Load in "filler sprite". Its some funny GBA window stuff :) to bypass the "cave flash" issue
...
static void DynamicallyLoadStartMenuIcon(u8 index)
{
    ...
    case MENU_ACTION_DEXNAV:
        LoadStartMenuIcon(8, index);
        break;
    case MENU_ACTION_BOX:
        LoadStartMenuIcon(9, index);
        break;
    case MENU_ACTION_POKENAV:
        LoadStartMenuIcon(10, index);
        break;
    default:
        break;
    }
