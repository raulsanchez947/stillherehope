...
void HandleAction_Switch(void)
{
    ...
    #ifndef NDEBUG
    MgbaPrintf(MGBA_LOG_DEBUG, "mon switch -1:%d", gSaveBlock2Ptr->playerRank);
    #endif
    gBattlerAttacker = gBattlerByTurnOrder[gCurrentTurnActionNumber];
    gBattle_BG0_X = 0;
    gBattle_BG0_Y = 0;
...
void HandleAction_UseItem(void)
{
    gSaveBlock2Ptr->playerRank -= 1;
    #ifndef NDEBUG
    MgbaPrintf(MGBA_LOG_DEBUG, "item used -1:%d", gSaveBlock2Ptr->playerRank);
    #endif
...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_SYRUP_BOMB:
            if ((gStatuses4[gActiveBattler] & STATUS4_SYRUP_BOMB) && (gBattleMons[gActiveBattler].hp != 0))
            {
                u16 battlerAbility = GetBattlerAbility(gActiveBattler);
                u32 battlerHoldEffect = GetBattlerHoldEffect(gActiveBattler, TRUE);

                gDisableStructs[gActiveBattler].syrupBombTimer--;
                if (gDisableStructs[gActiveBattler].syrupBombTimer == 0)
                {
                    gStatuses4[gActiveBattler] &= ~STATUS4_SYRUP_BOMB;
                    PREPARE_MOVE_BUFFER(gBattleTextBuff1, MOVE_SYRUP_BOMB);
                    gBattlescriptCurrInstr = BattleScript_WrapEnds;
                }
                else if (gDisableStructs[gActiveBattler].syrupBombTimer != 0)
                {
                    gBattlerTarget = gActiveBattler;
                    PREPARE_MOVE_BUFFER(gBattleTextBuff1, MOVE_SYRUP_BOMB);
...
static u8 ItemEffectMoveEnd(u32 battlerId, u16 holdEffect)
{
    ...
    case HOLD_EFFECT_ENIGMA_BERRY: // consume and heal if hit by super effective move
        effect = TrySetEnigmaBerry(battlerId);
        break;
...
u8 IsMonDisobedient(void)
{
    ...
    if (IsBattlerModernFatefulEncounter(gBattlerAttacker)) // only false if illegal Mew or Deoxys
    {
        ...
        {  
            if (gSaveBlock2Ptr->badges == 8)
            return 0;

            if (gSaveBlock2Ptr->badges >= 6)
                obedienceLevel = 88;
            else if (gSaveBlock2Ptr->badges >= 4)
                obedienceLevel = 56;
            else if (gSaveBlock2Ptr->badges >= 2)
                obedienceLevel = 38;
            else 
                obedienceLevel = 26;
        }
...
static u32 CalcMoveBasePowerAfterModifiers(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 updateFlags)
{
    ...
    case ABILITY_TRANSISTOR:
        if (moveType == TYPE_ELECTRIC)
            ...
            MulModifier(&modifier, UQ_4_12(5325 / 4096));
        #else
            MulModifier(&modifier, UQ_4_12(1.5));
        #endif
        break;
...
static u32 CalcFinalDmg(u32 dmg, u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, u16 typeEffectivenessModifier, bool32 isCrit, bool32 updateFlags)
{
    ...
    if (gBattleMoves[move].effect == EFFECT_COLLISION_COURSE && typeEffectivenessModifier >= UQ_4_12(2.0))
        MulModifier(&finalModifier, UQ_4_12(1.3333));
...
u16 GetTypeEffectiveness(struct Pokemon *mon, u8 moveType)
{
    ...
    {
        MulByTypeEffectiveness(&modifier, MOVE_POUND, moveType, 0, gSpeciesInfo[speciesDef].types[0], 0, FALSE);
        if (gSpeciesInfo[speciesDef].types[1] != gSpeciesInfo[speciesDef].types[0])
            MulByTypeEffectiveness(&modifier, MOVE_POUND, moveType, 0, gSpeciesInfo[speciesDef].types[1], 0, FALSE);
...
bool32 TryBattleFormChange(u8 battlerId, u16 method)
{
    ...
    else if (gBattleStruct->changedSpecies[side][monId] != SPECIES_NONE && gSaveBlock1Ptr->tx_Challenges_MegaEvo == FALSE)
    {
