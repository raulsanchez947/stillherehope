...
bool32 IsViableZMove(u8 battlerId, u16 move)
{
    ...
    if (gBattleTypeFlags & (BATTLE_TYPE_SAFARI | BATTLE_TYPE_WALLY_TUTORIAL | BATTLE_TYPE_ARENA))
        return FALSE;
...
bool32 MoveSelectionDisplayZMove(u16 zmove, u8 mode)
{
    u32 i;
    ...
    if (zmove != MOVE_NONE)
    {
        ...
        else if (mode == Z_MOVE_REFRESH_MOVE)
        {
            // Only clear the first and third moveslot. It isn't perfect but its good enough for now
            StringCopy(gDisplayedStringBattle, gText_EmptyString2);
            BattlePutTextOnWindow(gDisplayedStringBattle, i + 3);           // 1st moveslot
            BattlePutTextOnWindow(gDisplayedStringBattle, (i + 2) + 3);     // 3rd moveslot
        }
        ...
        if (IS_MOVE_STATUS(move))
        {
            ...
            AddTypeEffectivenessAndMoveNameToString(gDisplayedStringBattle + 2, gMoveNames[move]);
        ...
        else if (zmove == MOVE_EXTREME_EVOBOOST)
        {
            ...
            BattlePutTextOnWindow(gDisplayedStringBattle, B_WIN_MOVE_NAME_3);
            AddTypeEffectivenessAndMoveNameToString(gDisplayedStringBattle, GetZMoveName(zmove));
        ...
        BattlePutTextOnWindow(gDisplayedStringBattle, B_WIN_ZMOVE);//
