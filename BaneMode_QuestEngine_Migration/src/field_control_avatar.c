...
#if DEBUG_OVERWORLD_MENU == TRUE && DEBUG_OVERWORLD_IN_MENU == FALSE
    // Keep the overworld debug system compiled in for development, but do not
    // let it hijack the normal start-menu flow during regular play.
#endif
...
static const u8 *GetInteractedWaterScript(struct MapPosition *unused1, u8 metatileBehavior, u8 direction)
{
    ...
    {
        if (FlagGet(FLAG_OPEN_WORLD) && IsPlayerSurfingNorth() == TRUE)
        {
            if (FlagGet(FLAG_OPEN_WORLD) && IsPlayerSurfingNorth() == TRUE)            
...
static bool32 TrySetupDiveDownScript(void)
{
    ...
    if (FlagGet(FLAG_OPEN_WORLD) && TrySetDiveWarp() == 2)
    {
...
static bool32 TrySetupDiveEmergeScript(void)
{
    if (FlagGet(FLAG_OPEN_WORLD) && gMapHeader.mapType == MAP_TYPE_UNDERWATER && TrySetDiveWarp() == 1)
    {
...
static bool8 TryStartStepCountScript(u16 metatileBehavior)
{
    ...
    if (!(gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_FORCED_MOVE) && !MetatileBehavior_IsForcedMovementTile(metatileBehavior))
    {
        ...
        if (ShouldEggHatch())
        {
            ...
            #ifndef NDEBUG
            MgbaPrintf(MGBA_LOG_DEBUG, "egg hatched +5:%d", gSaveBlock2Ptr->playerRank);
            #endif
        ...
        if (ShouldDoRoxanneCall() == TRUE)
        {
            ...
            FlagSet(FLAG_ENABLE_ROXANNE_FIRST_CALL_OPW);
            return TRUE;
        }
...
static bool8 TryStartWarpEventScript(struct MapPosition *position, u16 metatileBehavior)
{
    ...
    if (warpEventId != WARP_ID_NONE && IsWarpMetatileBehavior(metatileBehavior) == TRUE)
    {
        StoreInitialPlayerAvatarState();
        SetupWarp(&gMapHeader, warpEventId, position);
        if (MetatileBehavior_IsEscalator(metatileBehavior) == TRUE && !IsMapRandmoized())
        {
            DoEscalatorWarp(metatileBehavior);
            return TRUE;
        }
        if (MetatileBehavior_IsLavaridgeB1FWarp(metatileBehavior) == TRUE && !IsMapRandmoized())
        {
            DoLavaridgeGymB1FWarp();
            return TRUE;
        }
        if (MetatileBehavior_IsLavaridge1FWarp(metatileBehavior) == TRUE && !IsMapRandmoized())
        {
            DoLavaridgeGym1FWarp();
            return TRUE;
        }
        if (MetatileBehavior_IsAquaHideoutWarp(metatileBehavior) == TRUE && !IsMapRandmoized())
        {
            DoTeleportTileWarp();
            return TRUE;
        }
        if (MetatileBehavior_IsUnionRoomWarp(metatileBehavior) == TRUE && !IsMapRandmoized())
        {
            DoSpinExitWarp();
            return TRUE;
        }
        if (MetatileBehavior_IsMtPyreHole(metatileBehavior) == TRUE && !IsMapRandmoized())
        {
            ScriptContext_SetupScript(EventScript_FallDownHoleMtPyre);
            return TRUE;
        }
        if (MetatileBehavior_IsMossdeepGymWarp(metatileBehavior) == TRUE && !IsMapRandmoized())
        {
            DoMossdeepGymWarp();
            return TRUE;
        }
...
EWRAM_DATA static u16 currMap = 0;
EWRAM_DATA static u16 currGroup = 0;
EWRAM_DATA static u16 wrapID = 0;
EWRAM_DATA static bool8 Found = FALSE;
EWRAM_DATA static u16 NewMapGroup = 0;
EWRAM_DATA static u16 NewMapNum = 0;
EWRAM_DATA static u16 NewWarpID = 0;

void softLock(void){
  SetWarpDestinationToMapWarp(currGroup, currMap, wrapID);
  DoWarp();
}
...
static void SetupWarp(struct MapHeader *unused, s8 warpEventId, struct MapPosition *position)
{
    ...
    currMap = mapHeader->events->warps[warpEvent->warpId].mapNum;
    currGroup = mapHeader->events->warps[warpEvent->warpId].mapGroup;
    wrapID = warpEventId;

    if (currMap != MAP_NUM(DYNAMIC)) {
        RandomizeMaps();
    }  
    
    if (FlagGet(FLAG_OPEN_WORLD) && gSaveBlock1Ptr->randomMap == 1 && Found == TRUE) { 

        SetWarpDestinationToMapWarp(NewMapGroup, NewMapNum, NewWarpID);
...
void RandomizeMaps(void) {
    u16 mapCount = ARRAY_COUNT(RandomMapsAndGroup);
    u16 i, j, k, temp;
    u16* index = (u16*)Alloc(mapCount * sizeof(u16));
    u16 desindex;
    u16 seed;
    Found = FALSE;
    ...
    if (gSaveBlock1Ptr->checks == TRUE && gSaveBlock1Ptr->SeedForShuffle[4] != 0) {
        seed = gSaveBlock1Ptr->SeedForShuffle[4];
    } else {
        seed = GetTrainerId(gSaveBlock2Ptr->playerTrainerId);
    }
    for (i = mapCount - 1; i > 0; i--) {
        j = RandomSeeded(seed, TRUE) % (i + 1);
    ...
    //     MgbaPrintf(MGBA_LOG_DEBUG, "{%d, %d, %d}", RandomMapsAndGroup[index[k]][0], RandomMapsAndGroup[index[k]][1], RandomMapsAndGroup[index[k]][2]);
    // }

    for (k = 0; k < mapCount; k++) {
        if (RandomMapsAndGroup[index[k]][0] == currMap && RandomMapsAndGroup[index[k]][1] == currGroup && RandomMapsAndGroup[index[k]][2] == wrapID) {
            if (k >= (mapCount / 2)) {
                desindex = k - (mapCount / 2); 
            } else {
                desindex = k + (mapCount / 2);
            }
            
            // Update the current map values
            NewMapNum = RandomMapsAndGroup[index[desindex]][0];
            NewMapGroup = RandomMapsAndGroup[index[desindex]][1];
            NewWarpID = RandomMapsAndGroup[index[desindex]][2];
            // MgbaPrintf(MGBA_LOG_DEBUG, "map = %d, group = %d, warp = %d, k = %d, desindex = %d\n currmap: %d, currgroup: %d, warp: %d", NewMapNum, NewMapGroup, NewWarpID, k, desindex, currMap, currGroup, wrapID);
            Found = TRUE;
            break; // Exit the loop after finding the map
        }
    }

    // if (!Found) {
    //     MgbaPrintf(MGBA_LOG_DEBUG, "******** FAILED ********");
    // }
...
// {
//     const struct MapHeader *mapHeader;
//     u16 i, map, group, j, count;
//     for (i = 0; i < ARRAY_COUNT(RandomMaps); i++) {
//         map = RandomMaps[i] & 0xFF;
//         group = (RandomMaps[i] >> 8) & 0xFF;
    

//         mapHeader = Overworld_GetMapHeaderByGroupAndId(group, map); 
//         count = mapHeader->events->warpCount;
//         for (j = 0; j < count; j++)
//         {
//             MgbaPrintf(MGBA_LOG_DEBUG, "{%d, %d, %d},", map, group, j);
//         }
