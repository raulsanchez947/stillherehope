    ...
    if (gBattleControllerExecFlags == 0)
    {
        if (gIsCriticalHit == TRUE && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
        {
            ...
            if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
                ...
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "crit +1:%d", gSaveBlock2Ptr->playerRank);
                #endif
            }            
...
static void Cmd_dofaintanimation(void)
{
    ...
    if (gBattleControllerExecFlags == 0)
    {
        ...
        if (GetBattlerSide(gActiveBattler) == B_SIDE_PLAYER) {
            gSaveBlock2Ptr->playerRank -= 1;
            #ifndef NDEBUG
            MgbaPrintf(MGBA_LOG_DEBUG, "player mon faint -1:%d", gSaveBlock2Ptr->playerRank);
            #endif  
        } else {
            gSaveBlock2Ptr->playerRank += 2;
            #ifndef NDEBUG
            MgbaPrintf(MGBA_LOG_DEBUG, "enemy mon faint +2:%d", gSaveBlock2Ptr->playerRank);
            #endif  
        }
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
...
static void Cmd_getexp(void)
{
    ...
    partysize = CalculatePlayerPartyCount();
    ...
    case 1: // calculate experience points to redistribute
        {
            u32 calculatedExp;
            s32 viaSentIn;
            for (viaSentIn = 0, i = 0; i < partysize; i++)
            {
                ...
                if (holdEffect == HOLD_EFFECT_EXP_SHARE || FlagGet(FLAG_SYS_EXP_SHARE) ||  gSaveBlock2Ptr->ExpShare == TRUE)
                    viaExpShare++;
            }
            ...
            if (B_TRAINER_EXP_MULTIPLIER <= GEN_7 && gBattleTypeFlags & BATTLE_TYPE_TRAINER)
                ...
                {
                    if (TX_EXP_MULTIPLER_ONLY_ON_NUZLOCKE_AND_RANDOMIZER)
                    {
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if ((holdEffect != HOLD_EFFECT_EXP_SHARE && !(gBattleStruct->sentInPokes & 1) && gSaveBlock2Ptr->ExpShare != TRUE)
             || GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPECIES_OR_EGG) == SPECIES_EGG)
            {
                *(&gBattleStruct->sentInPokes) >>= 1;
                ...
                gBattleMoveDamage = 0; // used for exp
                  ...
                  || GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) == GetCurrentPartyLevelCap())
            {
                *(&gBattleStruct->sentInPokes) >>= 1;
                gBattleScripting.getexpState = 5;
                gBattleMoveDamage = 0; // used for exp
            ...
            else
            {
                ...
                #else
                    if ((holdEffect == HOLD_EFFECT_EXP_SHARE || FlagGet(FLAG_SYS_EXP_SHARE) ||  gSaveBlock2Ptr->ExpShare == TRUE) && gBattleMoveDamage == 0)
                        gBattleMoveDamage += gExpShareExp;
                #endif
    ...
    case 5: // looper increment
        ...
        else
        {
            ...
            if (gBattleStruct->expGetterMonId < partysize)
...
static void Cmd_returntoball(void)
{
    ...
    MarkBattlerForControllerExec(gActiveBattler);
    TryBattleFormChange(gActiveBattler, FORM_CHANGE_BATTLE_SWITCH);
...
static void Cmd_setrain(void)
{
    ...
    else
    {
        if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
                gSaveBlock2Ptr->playerRank += 1;
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "train set +1:%d", gSaveBlock2Ptr->playerRank);
                #endif
    ...
    else
    {
        ...
        if (lands)
        {
            ...
            if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
                gSaveBlock2Ptr->playerRank += 1;
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "1HKO +1:%d", gSaveBlock2Ptr->playerRank);
                #endif
            } else
            {
                gSaveBlock2Ptr->playerRank -= 1;
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "enemy 1HKO -1:%d", gSaveBlock2Ptr->playerRank);
                #endif                
            }
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
...
static void Cmd_setsandstorm(void)
{
    ...
    else
    {
        if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
                gSaveBlock2Ptr->playerRank += 1;
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "sandstrom +1:%d", gSaveBlock2Ptr->playerRank);
                #endif
            } 
...
static void Cmd_setmist(void)
{
    ...
    else
    {
        if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
                gSaveBlock2Ptr->playerRank += 1;
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "mist +1:%d", gSaveBlock2Ptr->playerRank);
                #endif
            } 
...
static void Cmd_metronome(void)
{
    ...
    while (TRUE)
    {
        ...
        if (!gBattleMoves[gCurrentMove].metronomeBanned || gSaveBlock1Ptr->MeteronomeOnlyMode)
        {
            gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
            gBattlescriptCurrInstr = gBattleScriptsForMoveEffects[gBattleMoves[gCurrentMove].effect];
            gBattlerTarget = GetMoveTarget(gCurrentMove, NO_TARGET_OVERRIDE);
...
static void Cmd_trysetspikes(void)
{
    ...
    else
    {
        if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
                gSaveBlock2Ptr->playerRank += 1;
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "spike +1:%d", gSaveBlock2Ptr->playerRank);
                #endif
            }
...
static void Cmd_setsunny(void)
{
    ...
    else
    {
        if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
                gSaveBlock2Ptr->playerRank += 1;
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "sunny +1:%d", gSaveBlock2Ptr->playerRank);
                #endif
            } 
...
static void Cmd_sethail(void)
{
    ...
    else
    {
        if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
                gSaveBlock2Ptr->playerRank += 1;
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "hail +1:%d", gSaveBlock2Ptr->playerRank);
                #endif
            } 
...
static void Cmd_settoxicspikes(void)
{
    ...
    else
    {
        if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
                gSaveBlock2Ptr->playerRank += 1;
                #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "toxic spike +1:%d", gSaveBlock2Ptr->playerRank);
                #endif
            } 
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        if (odds > 254) // mon caught
        {
            gSaveBlock2Ptr->playerRank += 2;
            #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "caught mon +2:%d", gSaveBlock2Ptr->playerRank);
            #endif
            BtlController_EmitBallThrowAnim(BUFFER_A, BALL_3_SHAKES_SUCCESS);
            MarkBattlerForControllerExec(gActiveBattler);
        ...
        else // mon may be caught, calculate shakes
        {
            ...
            else if (gLastUsedItem == ITEM_LUXURY_BALL && gBattleResults.shinyWildMon)
            {
                shakes = maxShakes;
            }
            else
...
static void Cmd_trygivecaughtmonnick(void)
{
    ...
    case 2:
        if (!gPaletteFade.active)
        {
            ...
            gSaveBlock2Ptr->playerRank += 1;
            #ifndef NDEBUG
                MgbaPrintf(MGBA_LOG_DEBUG, "nickname +1:%d", gSaveBlock2Ptr->playerRank);
            #endif
    ...
    case 4:
        if (CalculatePlayerPartyCount() == GetMaxPartySize()) 
            gBattlescriptCurrInstr = cmd->nextInstr;
        else if (typeChallenge != TX_CHALLENGE_TYPE_OFF && 
