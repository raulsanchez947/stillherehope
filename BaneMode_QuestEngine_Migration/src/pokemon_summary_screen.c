...
static void AppendEvolutionSummary(u8 *dest, size_t destSize);
static const u8 *GetEvolutionMethodLabel(u16 method, u16 param);
...
static void Task_HandleInput(u8 taskId)
{
    ...
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        else if (JOY_NEW(A_BUTTON))
        {
            ...
            else if (sMonSummaryScreen->currPageIndex == PSS_PAGE_SKILLS && FlagGet(FLAG_SHOW_IV_AND_EV))
            {
                PlaySE(SE_SELECT);
...
static void BufferMonTrainerMemo(void)
{
    ...
    else
    {
        ...
        AppendEvolutionSummary(gStringVar4, sizeof(gStringVar4));
        Free(metLevelString);
...
static void AppendEvolutionSummary(u8 *dest, size_t destSize)
{
    ...
    if (evolutions == NULL || evolutions[0].method == EVO_NONE)
    {
        StringAppend(dest, COMPOUND_STRING("\nNo further evolution."));
        return;
    }

    for (i = 0; i < EVOS_PER_MON && evolutions[i].method != EVO_NONE; i++)
        methodCount++;

    StringCopy(line, COMPOUND_STRING("\n"));
    ...
    case EVO_LEVEL_NATURE_LOW_KEY:
        StringAppend(line, COMPOUND_STRING("Evolves at Lv."));
        ConvertIntToDecimalStringN(gStringVar1, evolutions[0].param, STR_CONV_MODE_LEFT_ALIGN, 3);
    ...
    case EVO_ITEM_NIGHT:
        ...
        break;
    case EVO_TRADE:
        StringAppend(line, COMPOUND_STRING("Evolves by trade."));
        break;
    case EVO_TRADE_ITEM:
        CopyItemName(evolutions[0].param, itemName);
    ...
    case EVO_MOVE:
        ...
        break;
    default:
        StringAppend(line, GetEvolutionMethodLabel(evolutions[0].method, evolutions[0].param));
        break;
    }
...
static const u8 *GetEvolutionMethodLabel(u16 method, u16 param)
{
    static const u8 sSpecialPlaceText[] = _("\nEvolves in a special place.");
    static const u8 sSpecialAreaText[] = _("\nEvolves in a special area.");
    static const u8 sSeasonalText[] = _("\nEvolves in a certain season.");
    static const u8 sBeautyText[] = _("\nEvolves with high beauty.");
    static const u8 sHeldItemText[] = _("\nEvolves with a held item.");
    static const u8 sSpecialMethodText[] = _("\nEvolution method special.");

    switch (method)
    {
    case EVO_SPECIFIC_MAP:
        return sSpecialPlaceText;
    case EVO_MAPSEC:
        return sSpecialAreaText;
    case EVO_SEASONAL:
        return sSeasonalText;
    case EVO_BEAUTY:
        return sBeautyText;
    case EVO_ITEM_HOLD_DAY:
    case EVO_ITEM_HOLD_NIGHT:
    case EVO_ITEM_HOLD:
        return sHeldItemText;
    default:
        (void)param;
        return sSpecialMethodText;
    }
}
...
static void PrintSkillsPage(void)
{
    ...
    else if (sMonSummaryScreen->currStatIndex == 1)
    {
       ...
       if (IsStatHyperTrained(mon, 0))
           StringAppend(gStringVar1, gText_Stats_EV_Plus1);
    }
    else
    {
...
static void PrintInfoBar(u8 pageIndex, bool8 detailsShown)
{
    ...
    switch (pageIndex)
    {
        ...
        case PSS_PAGE_SKILLS:
            if (!FlagGet(FLAG_SHOW_IV_AND_EV))
            {
