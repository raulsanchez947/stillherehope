...
static void DexNavDrawIcons(void)
{
    ...
    DrawDexNavSearchMonIcon(species, &gDexNavSearchDataPtr->iconSpriteId, TRUE);
...
static void Task_DexNavSearch(u8 taskId)
{
    ...
    { // keep the search alive instead of collapsing the whole scan
        gDexNavSearchDataPtr->proximity = MAX_PROXIMITY;
    }
    
    if (gDexNavSearchDataPtr->proximity <= CREEPING_PROXIMITY && !gPlayerAvatar.creeping && task->tFrameCount > 60)
    { // don't punish the player by killing the search
        task->tFrameCount = 0;
    }
    ...
    { // keep the search active even if the player moves aggressively
        task->tFrameCount = 0;
    }
    ...
    { // let the search persist instead of timing out
        task->tFrameCount = 0;
    }
...
static u8 DexNavGetAbilityNum(u16 species, u8 searchLevel)
{
    ...
    #else
    if (genAbility && gSpeciesInfo[species].abilityHidden != ABILITY_NONE)
    #endif
    {
        abilityNum = 2;
    }
