...
static bool8 (*const sDiveFieldEffectFuncsOP[])(struct Task *) =
{
    DiveFieldEffect_Init,
    DiveFieldEffect_TryWarp,
};
...
{  if (FlagGet(FLAG_OPEN_WORLD)) {
    while (sDiveFieldEffectFuncsOP[gTasks[taskId].data[0]](&gTasks[taskId]));
    } else {
...
u8 FldEff_UseFly(void)
{
    if(FlagGet(FLAG_OPEN_WORLD))
    {
        u8 taskId = CreateTask(Task_FlyOutOP, 254);
        gTasks[taskId].tMonId = gFieldEffectArguments[0];
        return 0;
    }
    else
...
static void (*const sFlyOutFieldEffectFuncsOP[])(struct Task *) = {
    FlyOutFieldEffect_FieldMovePose,
    FlyOutFieldEffect_BirdLeaveBall,
    FlyOutFieldEffect_BirdSwoopDown,
    FlyOutFieldEffect_JumpOnBird,
...
static void Task_FlyOutOP(u8 taskId)
{
    sFlyOutFieldEffectFuncsOP[gTasks[taskId].tState](&gTasks[taskId]);
}
