...
bool8 ScrCmd_addrankpoints(struct ScriptContext *ctx)
{
    gSaveBlock2Ptr->playerRank += ScriptReadByte(ctx);
    return FALSE;
}

bool8 ScrCmd_subrankpoints(struct ScriptContext *ctx)
{
    gSaveBlock2Ptr->playerRank -= ScriptReadByte(ctx);
    return FALSE;
}
...
bool8 ScrCmd_checkpartymove(struct ScriptContext *ctx)
{
    ...
    gSpecialVar_Result = PARTY_SIZE;
    if (!FlagGet(FLAG_OPEN_WORLD) && CheckBagHasItem(itemId, 1))
    {
        if (moveId != MOVE_SURF && moveId != MOVE_WATERFALL && moveId != MOVE_DIVE)
        {
            gSpecialVar_0x8004 = GetMonData(&gPlayerParty[0], MON_DATA_SPECIES, NULL);
            gSpecialVar_Result = 0;
        } else {
        for (i = 0; i < PARTY_SIZE; i++) {
                ...
                if (!species)
                    break;
                if (CanTeachMove(&gPlayerParty[i], moveId) == CAN_LEARN_MOVE || CanTeachMove(&gPlayerParty[i], moveId) == ALREADY_KNOWS_MOVE) 
                {
                    gSpecialVar_Result = i;
...
bool8 ScrCmd_setwildbattle(struct ScriptContext *ctx)
{
    ...
    FlagClear(FLAG_ROAMING_MON);

    return FALSE;
}

void setwildbossbattle(void)
{
    u16 species = VarGet(VAR_TEMP_6);
    u8 level = VarGet(VAR_TEMP_7);
    u16 item = VarGet(VAR_TEMP_8);
