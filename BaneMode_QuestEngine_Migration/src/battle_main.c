...
u32 GeneratePartyHash(const struct Trainer *trainer, u32 i)
{
    const u8 *buffer;
    u32 n;
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags, u16 trainerNum)
{
    ...
    u8 TpartySize = trainer->partySize - GymLeaderPartySize[gSaveBlock2Ptr->badges];
...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum, bool8 firstTrainer)
{
    ...
    // {
        retVal = DynamicTarinerParty(party, &gTrainers[trainerNum], firstTrainer, gBattleTypeFlags, trainerNum);
    // } else {
...
void CreateTrainerPartyForPlayer(void)
{
    ...
    CreateNPCTrainerPartyFromTrainer(gPlayerParty, &gTrainers[gSpecialVar_0x8004], TRUE, BATTLE_TYPE_TRAINER, gTrainerBattleOpponent_A);
}
...
static void ApplyBaneModeRoutePressure(void)
{
    ...
    // so battle intro and overworld visuals stay in sync.
}
...
static void ApplyBaneModeBattlePressure(void)
{
    ...
    case TRAINER_TATE_AND_LIZA_5:
        gFieldStatuses = STATUS_FIELD_PSYCHIC_TERRAIN | STATUS_FIELD_TERRAIN_PERMANENT | STATUS_FIELD_TRICK_ROOM;
        gFieldTimers.trickRoomTimer = 0xFF;
        break;
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!(gBattleControllerExecFlags & ((gBitTable[gActiveBattler]) | (0xF << 28) | (gBitTable[gActiveBattler] << 4) | (gBitTable[gActiveBattler] << 8) | (gBitTable[gActiveBattler] << 12))))
            {
                ...
                case B_ACTION_SAFARI_BALL:
                    ...
                    else if (IsNuzlockeActive() || IsOneTypeChallengeActive()) 
                    {
...
static void HandleEndTurn_BattleWon(void)
{
    ...
    #ifndef NDEBUG
    MgbaPrintf(MGBA_LOG_DEBUG, "won +5:%d", gSaveBlock2Ptr->playerRank);
    #endif
...
static void HandleEndTurn_BattleLost(void)
{
    ...
    gSaveBlock2Ptr->playerRank -= 5;
    #ifndef NDEBUG
    MgbaPrintf(MGBA_LOG_DEBUG, "lost -5:%d", gSaveBlock2Ptr->playerRank);
    #endif
...
static void HandleEndTurn_RanFromBattle(void)
{
    ...
    #ifndef NDEBUG
    MgbaPrintf(MGBA_LOG_DEBUG, "ran -2:%d", gSaveBlock2Ptr->playerRank);
    #endif
