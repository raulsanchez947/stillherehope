...
#define RAID_REPEL_STEP_COUNT 0x7FFF

static const u8 sText_RaidEnabled[] = _("{PLAYER} activated\nthe RAID field.\pWild POKeMON will keep their distance.{PAUSE_UNTIL_PRESS}");
static const u8 sText_RaidDisabled[] = _("{PLAYER} lowered\nthe RAID field.\pWild POKeMON can close in again.{PAUSE_UNTIL_PRESS}");
static const u8 sText_PortablePcDisabled[] = _("LEAGUE CORE:\nPortable PC access is disabled\nfor this challenge run.{PAUSE_UNTIL_PRESS}");
...
void DisplayCannotUseItemMessage(u8 taskId, bool8 isUsingRegisteredKeyItemOnField, const u8 *str) //static 
{
...
{
    if (!FlagGet(FLAG_SYS_EXP_SHARE) && gSaveBlock2Ptr->ExpShare == FALSE)
    {
...
void ItemUseOutOfBattle_Itemfinder(u8 var)
{
    ...
    #ifndef NDEBUG
    MgbaPrintf(MGBA_LOG_DEBUG, "item finder +1:%d", gSaveBlock2Ptr->playerRank);
    #endif
...
void ItemUseOutOfBattle_PokemonBoxLink(u8 taskId)
{
    ...
    {
        DisplayCannotUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem, sText_PortablePcDisabled);
        return;
    }
...
void ItemUseOutOfBattle_Raid(u8 taskId)
{
    u16 repelValue = VarGet(VAR_REPEL_STEP_COUNT);

    PlaySE(SE_REPEL);
    if (!IS_LAST_USED_LURE(repelValue) && REPEL_LURE_STEPS(repelValue) == RAID_REPEL_STEP_COUNT)
    {
        VarSet(VAR_REPEL_STEP_COUNT, 0);
        if (!InBattlePyramid())
            DisplayItemMessage(taskId, FONT_NORMAL, sText_RaidDisabled, CloseItemMessage);
        else
            DisplayItemMessageInBattlePyramid(taskId, sText_RaidDisabled, Task_CloseBattlePyramidBagMessage);
    }
    else
    {
        VarSet(VAR_REPEL_STEP_COUNT, RAID_REPEL_STEP_COUNT);
#if VAR_LAST_REPEL_LURE_USED != 0
        VarSet(VAR_LAST_REPEL_LURE_USED, ITEM_REPEL);
#endif
        if (!InBattlePyramid())
            DisplayItemMessage(taskId, FONT_NORMAL, sText_RaidEnabled, CloseItemMessage);
        else
            DisplayItemMessageInBattlePyramid(taskId, sText_RaidEnabled, Task_CloseBattlePyramidBagMessage);
    }
}
...
static bool32 CannotUseBagBattleItem(u16 itemId)
{
    ...
    if (battleUsage == EFFECT_ITEM_ESCAPE && FlagGet(FLAG_NO_ESCAPE))
    {
        cannotUse++;
    }
    // Poke Balls
...
void ItemUseOutOfBattle_CelesticaFlute(u8 taskId)
{
	s16* data = gTasks[taskId].data;
...
void ItemUseOutOfBattle_PokeVial(u8 taskId)
{
    ...
    if (vialUsages >= vialUsagesMax || FlagGet(FLAG_LOCK_BOX)){
